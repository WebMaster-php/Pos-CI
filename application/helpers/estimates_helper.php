...
function get_estimates_percent_by_status($status, $total_estimates = '')
{
    ...
    if (!is_numeric($status)) {
        if ($status == 'not_sent') {
            $total_by_status = total_rows('tblestimates', 'sent=0 AND portfolio_id = ' . $CI->session->userdata('portfolio_id') . ' AND status NOT IN(2,3,4) ' . (!$has_permission_view ? ' AND addedfrom=' . get_staff_user_id() : ''));
        }
    } else {
        $where = array(
            'status' => $status,
