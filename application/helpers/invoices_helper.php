...
function get_invoices_percent_by_status($status, $total_invoices = '')
{
		...
		*/
		...
		$total_by_status = total_rows('tblinvoices', $where_temp.' AND sent=0 AND status NOT IN(2,5)' . (!$has_permission_view ? ' AND addedfrom=' . get_staff_user_id() : ''));
		}
		} else {
		// adding $where_temp for portfolio check
		$total_by_status = total_rows('tblinvoices', $where_temp.' AND status = ' . $status . ' AND status NOT IN(5)' . (!$has_permission_view ? ' AND addedfrom=' . get_staff_user_id() : ''));
		}
