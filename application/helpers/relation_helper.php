...
function get_relation_data($type, $rel_id = '', $connection_type = '', $connection_id = '')
{
    ...
    if ($type == 'customer' || $type == 'customers') {
		...
		if($check == '')
            {
                $where_clients .= " AND tblclients.portfolio_id='".$CI->session->userdata('portfolio_id')."'";
            }
	...
	elseif ($type == 'saleopp') {
        if ($rel_id != '') {
            ...
            $data = $CI->sales_model->get($rel_id);
        } else {
            ...
            $search = $CI->misc_model->_search_projects_so($q, 0, $where_projects);
            $data   = $search['result'];
        }
    }
	//bilal code end
...
function get_relation_values($relation, $type)
{
    ...
    if ($type == 'customer' || $type == 'customers') {
        ...
        if (is_array($relation)) { 
            //$userid = isset($relation['company_id']) ? $relation['company_id'] : $relation['relid'];
			if(isset($relation['company_id']))
			{
				$userid = $relation['company_id'];
			}
			else if(isset($relation['relid']))
			{
				$userid =  $relation['relid'];
			}
			else 
			{
				$userid =  '';
...
function get_relation_values_avis($relation, $type)
{
    if ($relation == '') {
        return array(
            'name' => '',
