...
function get_customer_profile_tabs($customer_id)
{
  ...
  array(
	...
	array(
		'name'=>'saleopp',
		'url'=>admin_url('clients/client/'.$customer_id.'?group=saleopp'),
		'icon'=>'fa fa-bars',
		'lang'=>_l('saleopp'),
		'visible'=>true,
		'order'=>18,
...
function is_primary_contact($contact_id = '')
{
    ...
    if (total_rowss('tblcontacts', array(
        'id' => $contact_id,
        )) > 0) {
        return true;
    }
...
{
    $CI =& get_instance();
	$CI->db->select('tblcontacts.*, cs1.*');
	$CI->db->from('tblcontacts');
	$CI->db->join('tblcontacts_rel_clients as cs1', 'cs1.contact_id = tblcontacts.id', 'LEFT');
    $CI->db->where('tblcontacts.userid', $userid);
    $CI->db->where('cs1.is_primary', 1);
    $row = $CI->db->get()->row();
...
{	
    ...
    foreach ($permissions as $_permission) {
        if ($_permission['short_name'] == $permission) {
            $response = total_rows('tblcontactpermissions', array(
...
function login_as_client($id)
{
    ...
    // $CI->db->from('tblcontacts');
    // $d = $CI->db->get()->result_array();
    //  echo "<pre>"; print_r($d); exit('w');

    $CI->db->select('tblcontacts.id')
    ->join('tblcontacts_rel_clients as cc',  'cc.contact_id = tblcontacts.id ')
    ->where('cc.company_id', $id)
    ->where('is_primary', 1);

    $primary = $CI->db->get('tblcontacts')->result_array();
    // echo "<pre>"; print_r($primary); exit('w');  
    if (!$primary) {
...
{
	$CI =& get_instance();
	$CI->db->select('tcl.company as company, tcl.userid as userid');
	$CI->db->from('tblclients AS tcl');// I use aliasing make joins easier
	$CI->db->join('tblcontacts_rel_clients AS tclr', 'tclr.company_id = tcl.userid', 'LEFT');
	//$CI->db->join('tblcontacts AS tco', 'tco.id = tclr.company_id', 'LEFT');
	$CI->db->where('tclr.contact_id',$id);
...
{
	$CI =& get_instance();
	$CI->db->select('tco.firstname as firstname, tco.lastname as lastname, tco.email as email, tco.phonenumber as phonenumber, tco.id as contactsid');
	$CI->db->from('tblcontacts_rel_clients AS tclr');// I use aliasing make joins easier
	//$CI->db->join('tblcontacts_rel_clients AS tclr', 'tclr.company_id = tcl.userid', 'LEFT');
	$CI->db->join('tblcontacts AS tco', 'tco.id = tclr.contact_id', 'LEFT');
	$CI->db->where('tclr.company_id',$id);
	$CI->db->where('tclr.is_primary',1);
	return $CI->db->get()->row();
}
function getServicesNone($name = '', $id = '')    
{
    $CI =& get_instance();
    $CI->db->select('tc.name', 'tci.');
    $CI->db->from('tblcustomersservices AS tc');
    $CI->db->join('tblcustomerservices_in AS tci', 'tc.id = tci.serviceid', 'LEFT');
    $CI->db->join('tblclients AS tcl', 'tci.customer_id = tcl.userid', 'LEFT');
    $CI->db->where('tc.name',$name);
    $CI->db->where('tci.customer_id',$id);
    $CI->db->where('tcl.portfolio_id !=',1);
    return $CI->db->get()->result_array();
}
// SELECT count(*) FROM `tblclients` LEFT JOIN `tblcustomerservices_in` ON tblclients.userid = tblcustomerservices_in.customer_id 
// RIGHT JOIN tblcustomersservices ON tblcustomerservices_in.serviceid = tblcustomersservices.id WHERE tblcustomersservices.name = 'none'


function getCounterNone($none)    
{
    // exit('ijijijijilksdbfs');
    $CI =& get_instance();
    $CI->db->select('*');
    $CI->db->from('tblclients AS c');
    $CI->db->join('tblcustomerservices_in AS csi', 'c.userid = csi.customer_id', 'LEFT');
    $CI->db->join('tblcustomersservices AS cs', 'csi.serviceid = cs.id', 'LEFT');
    $CI->db->where($none);
    return $CI->db->get()->num_rows();
}
function getCounterNoneContacts($none)    
{
    $CI =& get_instance();
    $CI->db->select('*');
    $CI->db->from('tblcontacts AS c');
    $CI->db->join('tblcustomerservices_in AS csi', 'c.userid = csi.customer_id', 'LEFT');
    $CI->db->join('tblcustomersservices AS cs', 'csi.serviceid = cs.id', 'LEFT');
    $CI->db->where($none);
    return $CI->db->get()->num_rows();
 ...
 // <!-- faisal code starts-->
function get_portfolio_premission($id)
{
    $data_array = array('staffid' => $id);
    $CI =& get_instance();
    $CI->db->select('*');
    $CI->db->from('tblstaff');
    $CI->db->where($data_array);
...
function update_portfolio_id($id, $portfolio_id)
{
    // echo $id; echo $portfolio_id; exit;
    $update_arr= 0;
    // $data_array = array('tblinvoices','tblproposals', 'tblcreditnotes');
    $data_array = array('portfolio_id'=> $portfolio_id);
    $arr = array();
    $CI =& get_instance();
    $CI->db->where('clientid' , $id);
    $CI->db->update('tblinvoices' , $data_array);
    if($CI->db->affected_rows() > 0){
        $update_arr++;
        $arr['invoice'] =true;
    }
    $CI->db->where('userid' , $id);
    $CI->db->update('tblcontacts' , $data_array);
    if($CI->db->affected_rows() > 0){
        $update_arr++;
        $arr['tblcontacts'] =true;
    }
    $CI->db->where('account' , $id);
    $CI->db->update('tblinventoryadd' , $data_array);
    if($CI->db->affected_rows() > 0){
        $update_arr++;
        $arr['tblinventoryadd'] =true;
    }
    $CI->db->where('rel_id' , $id);
    $CI->db->update('tblproposals' , $data_array);
    if($CI->db->affected_rows() > 0){
        $arr['tblproposals'] =true;
        $update_arr++;
    }
    $CI->db->where('clientid' , $id);
    $CI->db->update('tblcreditnotes' , $data_array);
    if($CI->db->affected_rows() > 0){
        $arr['tblcreditnotes'] =true;
        $update_arr++;
    }
    $CI->db->where('clientid' , $id);
    $CI->db->update('tblestimates' , $data_array);
    if($CI->db->affected_rows() > 0){
        $arr['tblestimates'] =true;
        $update_arr++;
    }
    $CI->db->where('client' , $id);
    $CI->db->update('tblcontracts' , $data_array);
    if($CI->db->affected_rows() > 0){
        $arr['tblcontracts'] =true;
        $update_arr++;
    }
    $CI->db->where('clientid' , $id);
    $CI->db->update('tblexpenses' , $data_array);
    if($CI->db->affected_rows() > 0){
        $arr['tblexpenses'] =true;
        $update_arr++;
    }
    $CI->db->where('clientid' , $id);
    $CI->db->update('tblprojects' , $data_array);
    if($CI->db->affected_rows() > 0){
        $arr['tblprojects'] =true;
        $update_arr++;
    }
    $CI->db->where('rel_id' , $id);
    $CI->db->update('tblstafftasks' , $data_array);
    if($CI->db->affected_rows() > 0){
        $arr['tblstafftasks'] =true;
        $update_arr++;
    }
    $CI->db->where('userid' , $id);
    $CI->db->update('tbltickets' , $data_array);
    if($CI->db->affected_rows() > 0){
        $arr['tbltickets'] =true;
        $update_arr++;
    }
    $CI->db->where('rel_id' , $id);
    $CI->db->update(' tblreminders' , $data_array);
    if($CI->db->affected_rows() > 0){
        $arr['tblreminders'] =true;
        $update_arr++;
    }
    if($update_arr){
        // echo "<pre>"; print_r($arr);  
        // echo $update_arr; echo ' ';exit('BIg B '); 
        return true; 
    }
    else{
        return false;     
    ...
    function getServices($id)
    {
        $CI =& get_instance();
        $CI->db->select('*');
        $CI->db->from('tblcustomerservices_in AS csi');
        $CI->db->join('tblcustomersservices AS cs', 'csi.serviceid = cs.id', 'LEFT');
        $CI->db->where('csi.customer_id',  $id);
        return $CI->db->get()->result_array();
    }
    function get_client_portfolio($id){
        ...
        return $res[0]['portfolio_id'];

    }
    //sajid code start
function get_company_ids_by_contact($id = ''){
   
    $CI =& get_instance();
    $CI->db->select('company_id');
    $CI->db->from('tblcontacts_rel_clients');
    $CI->db->where('tblcontacts_rel_clients.contact_id', $id);
    $res = $CI->db->get()->result_array();
    $comp = '';
    if(!empty($res)){
        foreach ($res as $value) {
            $comp .= $value['company_id'].',';
        }
        $ids = rtrim($comp,",");   
        return $ids;
    }
...
function get_size_for_versieats($id = ''){
    $CI =& get_instance();
    $CI->load->model('clients_model');
    return $CI->clients_model->get_size_for_price($id);

}
function get_contacts_email_tickets($id = '')
    {
        $CI =& get_instance();
        $CI->db->select('tblcontacts.email');
        $CI->db->from('tblcontacts');
        $CI->db->where('tblcontacts.id', $id);
        $res = $CI->db->get()->result_array();
        return $res[0]['email'];
    }
function get_all_contact_company($id = '')
    {
        $CI =& get_instance();
        $CI->load->model('clients_model');
        $con = $CI->clients_model->get_compny_contacts($id);   
        return $con;
    }  
