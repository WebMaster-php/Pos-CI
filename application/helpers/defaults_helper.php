...
{ 
    return 'CASE company WHEN "" THEN (SELECT CONCAT(firstname, " ", lastname) FROM tblcontacts LEFT JOIN tblcontacts_rel_clients AS cs1 ON cs1.contact_id = tblcontacts.id WHERE userid = tblclients.userid and cs1.is_primary = 1) ELSE company END as company';
}
...
//bilal code start
function get_sql_select_task_assignees_ids_so()
{
    return '(SELECT GROUP_CONCAT(staffid SEPARATOR ",") FROM tblstafftaskassignees WHERE taskid=tblstafftasks.id ORDER BY tblstafftaskassignees.staffid)';
}
//bilal code end
