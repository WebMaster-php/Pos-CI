...
function is_admin($staffid = '')
{
	...
	function getAdmin(){
		//for customer
		$CI =& get_instance();
		$CI->db->select('*')->where('staffid', $CI->session->userdata('staff_user_id'))->where('role',3);
		return $CI->db->count_all_results('tblstaff') > 0 ? true : false;
	}
/**
 * Is user logged in
 ...
 * @return boolean
 */
function has_permission_hidden($staffid = '')
{
    // echo $staffid; exit;
    $CI =& get_instance();
    $CI->db->select("hidden_items");
    $CI->db->from('tblstaff');
...
function getAnouncements()
{
    $CI =& get_instance();
    if (is_client_logged_in()) {
        $val = contacts_by_portfolio(); 
        $CI->db->select('*');
		$CI->db->from('tblannouncements');
		//$CI->db->join('announcement_status', 'tblannouncements.announcementid = announcement_status.announcement_id','left');
		$CI->db->where('showtousers', 1);
        $CI->db->where('portfolio_id', $val);
...
function getAnounceStatus_($announce_id)
{
	
    $CI =& get_instance();
    if (is_client_logged_in()) {
		$user_id = $CI->session->userdata('contact_user_id');
        $CI->db->select('*');
		$CI->db->from('announcement_status');		
		$CI->db->where('announcement_id', $announce_id);
...
function getAnounceStatusCount_()
{
	$counter = 0;
	if(!empty(getAnouncements()))
	{
		$CI =& get_instance();
		$user_id = $CI->session->userdata('contact_user_id');
		
		foreach(getAnouncements() as $ann){
			
			if (is_client_logged_in()) 
			{	
				$CI->db->select('*');
				$CI->db->from('announcement_status');		
				$CI->db->where('announcement_id', $ann['announcementid']);
				$CI->db->where('user_id', $user_id);
...
function get_primary_contacts()
{
    ...
    if (is_client_logged_in()) {
		...
		$CI->db->where('contact_id', $user_id);
