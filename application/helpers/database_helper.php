...
function get_staff_full_name_for_tickets($assignee = '')
{
    $CI =& get_instance();
    $assignee = explode(',' ,  $assignee);
    $strassign = '';
    foreach ($assignee as $assig) {
        $CI->db->where('staffid', $assig);
        $staff = $CI->db->select('firstname,lastname')->from('tblstaff')->get()->row();
        if($staff){
...
function add_notification($values)
{
	...
	if( !null == $CI->session->userdata("portfolio_id"))
		{
			$data['portfolio_id'] = $CI->session->userdata("portfolio_id");
		}
...
//sajid code start
function total_rows_avis($table, $where = array())
{
    $CI =& get_instance(); 
    $id = $where['contactid'];
    $status = $where['status'];
...
// sajid code end
function total_rows_con($table, $where = array())
{
	$CI =& get_instance(); 
	if (is_array($where)) {
        if (sizeof($where) > 0) {
			$CI->db->join('tblclients as cs1', 'cs1.userid = '.$table.'.userid', 'LEFT');
			$CI->db->where($table . '.' . $where);
			$CI->db->where('tblclients.portfolio_id' , $CI->session->userdata('portfolio_id'));
        }
    } elseif (strlen($where) > 0) {
        $CI->db->join('tblclients ', 'tblclients.userid = ' . $table . '.userid' , 'LEFT');
		$CI->db->where($table . '.' . $where);
		$CI->db->where($table . '.portfolio_id' , $CI->session->userdata('portfolio_id'));
    }
    return $CI->db->count_all_results($table);
}

function total_rowss($table, $where = array())
{
    $CI =& get_instance();
    if (is_array($where)) {
        if (sizeof($where) > 0) {
			$CI->db->join('tblcontacts_rel_clients as cs1', 'cs1.contact_id = '.$table.'.id', 'LEFT');
            $CI->db->where($where);
...
function get_announcements_for_user($staff = true)
{
    ...
    $CI->db->where('portfolio_id='.$CI->session->userdata("portfolio_id"));
