...
$where = array();
//$where = array('WHERE );
$where = array('WHERE cs1.company_id ='.$client_id);
...
foreach ($rResult as $aRow) {
    ...
    $options = '';
    $options .= icon_btn('#', 'pencil-square-o', 'btn-default', array('onclick'=>'contact('.$aRow['company_id'].','.$aRow['id'].');return false;'));
    if (has_permission('customers', '', 'delete') || is_customer_admin($aRow['userid'])) {
        if ($aRow['is_primary'] == 0 || ($aRow['is_primary'] == 1 && $total_client_contacts == 1)) {
            $options .= icon_btn('clients/delete_contacts_rel_clients/'.$aRow['company_id'].'/'.$aRow['id'], 'remove', 'btn-danger _delete');
        }
    }
