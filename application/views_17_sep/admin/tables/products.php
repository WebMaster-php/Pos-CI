...
$join             = array(
    'LEFT JOIN tbltaxes t1 ON t1.id = tblproducts.tax',
    'LEFT JOIN tbltaxes t2 ON t2.id = tblproducts.tax2',
    'LEFT JOIN tblitems_groups ON tblitems_groups.id = tblproducts.group_id',
    );
...
foreach ($custom_fields as $key => $field) {
    ...
    array_push($join, 'LEFT JOIN tblcustomfieldsvalues as ctable_'.$key . ' ON tblproducts.id = ctable_'.$key . '.relid AND ctable_'.$key . '.fieldto="items_pr" AND ctable_'.$key . '.fieldid='.$field['id']);
}
...
foreach ($rResult as $aRow) {
    ...
    if (has_permission('items', '', 'delete')) {
        $options .= icon_btn('invoice_items/delete_product/' . $aRow['id'], 'remove', 'btn-danger _delete');
    }
    $row[] = $options;
