<?php echo form_open_multipart(admin_url('sales/upload_file/'.$project->id),array('class'=>'dropzone','id'=>'project-files-upload-as')); ?>
...
<a href="#" onclick="window.location.href = '<?php echo admin_url('sales/download_all_files/'.$project->id); ?>'; return false;" class="table-btn hide" data-table=".table-project-files"><?php echo _l('download_all'); ?></a>
...
<table class="table dt-table scroll-responsive table-project-files" data-order-col="7" data-order-type="desc">
  ...
  <tbody>
    <?php foreach($files as $file){
      $path = get_upload_path_by_type_so('saleopp') . $project->id . '/'. $file['file_name'];
      ?>
      <tr>
        <td>
        ...
        <td data-order="<?php echo $file['file_name']; ?>">
          <a href="#" onclick="view_project_file(<?php echo $file['id']; ?>,<?php echo $file['project_id']; ?>); return false;">
            <?php if(is_image(SALE_OPPORTUNITY_ATTACHMENTS_FOLDER .$project->id.'/'.$file['file_name']) || (!empty($file['external']) && !empty($file['thumbnail_link']))){
              echo '<div class="text-left"><i class="fa fa-spinner fa-spin mtop30"></i></div>';
              echo '<img class="project-file-image img-table-loading" src="#" data-orig="'.project_file_url_so($file,true).'" width="100">';
              echo '</div>';
            }
            echo $file['subject']; ?></a>
          </td>
          ...
          <td data-order="<?php echo $file['visible_to_customer']; ?>">
            ...
            ?>
            <div class="onoffswitch">
              <input type="checkbox" data-switch-url="<?php echo admin_url(); ?>sales/change_file_visibility" id="<?php echo $file['id']; ?>" data-id="<?php echo $file['id']; ?>" class="onoffswitch-checkbox" value="<?php echo $file['id']; ?>" <?php echo $checked; ?>>
         ...
         <td>
           ...
           <button type="button" data-toggle="modal" data-original-file-name="<?php echo $file['file_name']; ?>" data-filetype="<?php echo $file['filetype']; ?>" data-path="<?php echo SALE_OPPORTUNITY_ATTACHMENTS_FOLDER .$project->id.'/'.$file['file_name']; ?>" data-target="#send_file" class="btn btn-info btn-icon"><i class="fa fa-envelope"></i></button>
           <?php } ?>
           <?php if($file['staffid'] == get_staff_user_id() || has_permission('sales','','delete')){ ?>
           <a href="<?php echo admin_url('sales/remove_file/'.$project->id.'/'.$file['id']); ?>" class="btn btn-danger btn-icon _delete"><i class="fa fa-remove"></i></a>
           <?php } ?>
         </td>
       </tr>
       <?php } ?>
