  ...
  <div class="content">
          ...
          <div class="panel-body _buttons">
            <div class="row">
             ...
             <div class="col-md-4 text-right">
              ...
              <a href="#" onclick="new_task_from_relation_so(undefined,'saleopp',<?php echo $project->id; ?>); return false;" class="btn btn-info"><?php echo _l('new_task'); ?></a>
              <?php } ?>
              <?php
              $invoice_func = 'pre_invoice_project_so';
              ?>
              ...
              <div class="btn-group">
                ...
                <li>
                  <a href="<?php //echo admin_url('sales/project/'.$project->id); ?>">
                ...
                <?php } ?>
                <?php if(has_permission('sales','','create') || has_permission('sales','','edit')){ ?>
                <li class="divider"></li>
                <?php foreach($statuses as $status){
                  ...
                  <li>
                    ...
                    <a href="#"><?php echo _l('project_mark_as',$status['name']); ?></a>
                  </li>
                  <?php } ?>
                  <?php } ?>
                  <li class="divider"></li>
                  <?php if(has_permission('sales','','create')){ ?>
                  <li>
                   <a href="<?php //echo admin_url('sales/export_project_data/'.$project->id); ?>" target="_blank"><?php echo _l('export_project_data'); ?></a>
                 </li>
                 <?php } ?>
                 <?php if(is_admin()){ ?>
                 <li>
                  <a href="<?php //echo admin_url('sales/view_project_as_client/'.$project->id .'/'.$project->clientid); ?>" target="_blank"><?php echo _l('project_view_as_client'); ?></a>
                </li>
                <?php } ?>
                <?php if(has_permission('sales','','delete')){ ?>
                <li>
                  <a href="<?php //echo admin_url('sales/delete/'.$project->id); ?>" class="_delete">
    ...
    <div class="panel_s project-menu-panel">
      <div class="panel-body">
        ...
        <?php $this->load->view('admin/sales/project_tabs'); ?>
      </div>
    </div>
    ...
    <?php } else { ?>
    <?php if((has_permission('sales','','create') || has_permission('sales','','edit')) && $project->status == 1 && $this->sales_model->timers_started_for_project($project->id)){ ?>
...
<?php $this->load->view('admin/sales/milestone'); ?>
<?php $this->load->view('admin/sales/copy_settings'); ?>
<?php $this->load->view('admin/sales/_mark_tasks_finished'); ?>
<?php init_tail(); ?>
    ...
    function discussion_comments(selector,discussion_id,discussion_type){
     $(selector).comments({
       ...
       getComments: function(success, error) {
         $.get(admin_url + 'sales/get_discussion_comments/'+discussion_id+'/'+discussion_type,function(response){
           success(response);
         },'json');
       },
       postComment: function(commentJSON, success, error) {
         $.ajax({
           type: 'post',
           url: admin_url + 'sales/add_discussion_comment/'+discussion_id+'/'+discussion_type,
           data: commentJSON,
       ...
       putComment: function(commentJSON, success, error) {
         $.ajax({
           type: 'post',
           url: admin_url + 'sales/update_discussion_comment',
           data: commentJSON,
           success: function(comment) {
             comment = JSON.parse(comment);
             success(comment)
       ...
       deleteComment: function(commentJSON, success, error) {
         $.ajax({
           type: 'post',
           url: admin_url + 'sales/delete_discussion_comment/'+commentJSON.id,
           success: success,
           error: error
         });
       },
