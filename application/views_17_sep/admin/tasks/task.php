...
<div class="modal fade<?php if(isset($task)){echo ' edit';} ?>" id="_task_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"<?php if($this->input->get('opened_from_lead_id')){echo 'data-lead-id='.$this->input->get('opened_from_lead_id'); } ?>>
  <div class="modal-dialog" role="document">
    ...
    <div class="task-hours<?php if(isset($task) && $task->rel_type == 'project' && total_rows('tblprojects',array('id'=>$task->rel_id,'billing_type'=>3)) == 0){echo ' hide';}  if(isset($task) && $task->rel_type == 'salesopp' && total_rows('tblsales',array('id'=>$task->rel_id,'billing_type'=>3)) == 0){echo ' hide';} ?>">
      <?php $value = (isset($task) ? $task->hourly_rate : 0); ?>
      <?php echo render_input('hourly_rate','task_hourly_rate',$value); ?>
    </div>
    <div class="project-details<?php if($rel_type != 'project' || $rel_type != 'salesopp'){echo ' hide';} ?>">
      <div class="form-group">
    ...
    <div class="row">
	  ...
	  <!--imran arshad changes strart -->
	        <div class="col-md-6">

        <?php $value = (isset($task) ? _d($task->duedate) : '');
        if(isset($task->hard_date) =='' ){
            ?>
            <p  style="float: right; margin-bottom: 0px;padding-bottom: 0px;"><input type="checkbox" name="hard_date" value="1" <?php echo (isset($task->hard_date)==1)?'checked':'' ?>>Hard Due Date?</p>
            <?php
        }else{
         $currentUserId = isset($clientRequest) == false ? get_staff_user_id() : get_contact_user_id();
        if($currentUserId != $task->addedfrom &&  $task->addedfrom !='' &&  $task->hard_date !=''  &&  $task->hard_date ==1 ) {
            $disabled = 'disabled="disabled"';
        }
            ?>
            <p style="float: right; margin-bottom: 0px;padding-bottom: 0px;"><input type="checkbox" name="hard_date"
                                                                                    value="1" <?php echo ($task->hard_date == 1) ? 'checked' : '' ?> <?= isset($disabled) ?>>Hard
                Due Date?</p>
            <?php

        }
       $currentUserId = isset($clientRequest) == false ? get_staff_user_id() : get_contact_user_id();
       if($currentUserId != isset($task->addedfrom) &&  isset($task->addedfrom) !='' &&  isset($task->hard_date) !='' &&  isset($task->hard_date) ==1 ){
            $project_end_date_attrs['disabled'] = 'disabled';
       }
       ?>
        <?php echo render_date_input('duedate','task_add_edit_due_date',$value,$project_end_date_attrs); ?>
      </div>
	  <!--imran arshad changes end -->
  ...
  <div class="col-md-6">
    <div class="form-group">
      ...
      <select name="rel_type" class="selectpicker" id="rel_type" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
		...
		<option value="saleopp"
        <?php if(isset($task) || $this->input->get('rel_type')){if($rel_type == 'saleopp'){echo 'selected';}} ?>><?php echo _l('saleopp'); ?></option>
