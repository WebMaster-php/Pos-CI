...
class Emails_model extends CRM_Model
{
	...
	/**
     * Send email - No templates For Lagacy report
     * @param  string $email   email
     * @param  string $message message
     * @param  string $subject email subject
     * @return boolean
     */
	public function report_email($email, $subject,  $msg, $attach = '')
	{
		...
		if($attach){
			$this->email->attach($_SERVER['DOCUMENT_ROOT'] . '/versi Report.pdf');
		}
		if($this->email->send()) {
            return true;
		}
     ...
     * @return boolean
     */
    public function send_email_template($template_slug, $email, $merge_fields, $ticketid = '', $cc = '', $flag = '')
    {
        ...
        if($flag === 0){
            // unset($template->message);
            if (strpos($template->message, 'Ticket message:') !== false) {
                $template->message=str_replace("Ticket message:","",$template->message);
                    if (strpos($template->message, '{ticket_message}') !== false) {
                        $template->message=str_replace("{ticket_message}","",$template->message);      
                    }
        ...
        */
		...
		//if($template_slug == 'new-ticket-open-followers'){
		//	echo "<pre>"; print_r($template);  exit('here');  
		//}
		//echo $flag; echo "<pre>"; print_r($template);  exit('inkkjjjdjf'); 
