...
class Roles_model extends CRM_Model
{
	...
	public function get_permissions_port($id = '')
    {
			...
			foreach($ar as $a)
			{
				...
				{
					$a =  key($re); 
					$this->db->select('tblpermissions.*');
					$this->db->from('tblpermissions');
					$this->db->where('tblpermissions.shortname', $a);
		...
		$this->db->select('tblportfolio.* ');
			...
			foreach($ar as $a)
			{
				...
				while ($fruit_name = current($re)) 
				{
					...
					$this->db->where('tblpermissions.shortname', $a);
					$this->db->where('tblstaffpermissions.staffid', $id);
					$array[] = $this->db->get()->row();
					next($re);
				}						
			}
