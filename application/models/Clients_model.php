...
{   //echo "<pre>"; print_r($data); exit('endddd');
        $contact_data = array();
       ...
       // echo "<pre>"; print_r($contact_data); exit('cvcv');
        ...
        if ($userid) {
			...
			$this->db->update('tblleads', ['client_id' => $userid]);
        ...
        */
        if ($client_or_lead_convert_request == true) {
            // echo "<pre>"; print_r($contact_data); exit('why');  
            $this->db->select('*');
            $this->db->from('tblleadcontacts');
            $this->db->where('userid',$data['leadid']);
            $contact_result = $this->db->get()->result_array();
            // echo "<pre>"; print_r($contact_result); exit('jdsfhjdshhf');  
            if($contact_result){
               $contact_data['firstname'] = $contact_result[0]['first_name'];
               $contact_data['lastname'] = $contact_result[0]['last_name'];
               $contact_data['email'] = $contact_result[0]['lead_email'];
               $contact_data['title'] = $contact_result[0]['title'];
               $contact_data['phonenumber'] = $contact_result[0]['lead_phonenumber'];
               $contact_data['active'] = $contact_result[0]['active'];
               $contact_data['direction'] = $contact_result[0]['direction'];
               $contact_data['profile_image'] = $contact_result[0]['profile_image'];
               $contact_data['portfolio_id'] = $contact_result[0]['portfolio_id'];
               $contact_data['userid'] = $userid;
            }
            // echo "<pre>"; print_r($contact_data); exit('aaaaaaaaa');
        // $contact_id = $this->add_contact($contact_result[0], $userid, $client_or_lead_convert_request);
    ...
    public function add_contact($data, $customer_id, $not_manual_request = false)
    {
        ...
        {    
            ...
            // echo "<pre>"; print_r($data); exit('ashgfgddfjdfdf');
            $this->db->insert('tblcontacts', $data);
            // echo $this->db->last_query();
            // echo $sa;  echo "<pre>"; print_r($data); exit('ashgfgddfjdfdf');
 ...
 public function reports($formdata){
		...
		$datafrom = date('Y-m-d', strtotime($formdata['datefrom']));
		$dateto = date('Y-m-d', strtotime($formdata['dateto']));
				

		$date_start =$datafrom." "."00:00:00";  
        ...
        if($this->versi_con){
            ...
            $payment_tip = array();
            $tip_pay = 0;
            $tip_pay_count = 0; // this line new
            $delivery_charges = 0;
            $delivery_charges_count = 0;
            $tax = 0; 
            ...
            // for tax 
            // $query_tax =  "SELECT taxable_total FROM mt_order WHERE `date_created` BETWEEN  '".$date_start ."'  AND  '". $date_end ."'  AND merchant_id = ".$arr."  AND status = 'paid'  ";  // this line new
            // $resl_tax = $this->versi_con->query($query_tax)->result_array();
            // echo "<pre>"; print_r($resl_tax);  exit('lol'); 

            $res_ptype = array(0=>'cod', 1=>'mxm');
            
            foreach ($res_ptype as $ptype){
                
				$query_ptip =  "SELECT sum(taxable_total) FROM mt_order WHERE `date_created` BETWEEN  '".$date_start ."'  AND  '". $date_end ."'  AND merchant_id = ".$arr."   AND  payment_type = '". $ptype ."'  AND status = 'paid' ";
				$resltip = $this->versi_con->query($query_ptip)->result_array();
                
				$cart_query_ptip =  "SELECT count(cart_tip_value) , SUM(cart_tip_value) FROM mt_order WHERE `date_created` BETWEEN  '".$date_start ."'  AND  '". $date_end ."'  AND merchant_id = ".$arr."   AND  payment_type = '". $ptype ."' AND cart_tip_value !=''  AND status = 'paid' ";
				
				$resltip_tip = $this->versi_con->query($cart_query_ptip)->result_array();
					//echo "<pre>"; print_r($resltip_tip); exit('jijij'); 
				$resltip[0]['count(cart_tip_value)'] =  $resltip_tip[0]['count(cart_tip_value)'];
				$resltip[0]['SUM(cart_tip_value)'] =  $resltip_tip[0]['SUM(cart_tip_value)'];
				//echo "<pre>"; print_r($resltip); exit('jijij'); 
				
				$query_pdelivery =  "SELECT SUM(delivery_charge), count(delivery_charge) FROM mt_order WHERE `date_created` BETWEEN  '".$date_start ."'  AND  '". $date_end ."'  AND merchant_id = ".$arr."  AND delivery_charge != '0.0000'  AND  payment_type = '". $ptype ."'  AND status = 'paid' ";
				$resltip_delivery = $this->versi_con->query($query_pdelivery)->result_array();
				
				$resltip[0]['count(delivery_charge)'] =  $resltip_delivery[0]['count(delivery_charge)'];
				$resltip[0]['SUM(delivery_charge)'] =  $resltip_delivery[0]['SUM(delivery_charge)'];
				
				// echo $this->versi_con->last_query(); exit('lq');
                foreach ($resltip as  $resltips) {
                    $tip_pay += $resltips['SUM(cart_tip_value)']; 
                    $tip_pay_count += $resltips['count(cart_tip_value)']; // this line new
                    $delivery_charges += $resltips['SUM(delivery_charge)'];
                    $delivery_charges_count += $resltips['count(delivery_charge)']; 
                    $tax +=  $resltips['sum(taxable_total)']; 
                ...
                // echo "<pre>"; print_r($tip_pay_count);  exit('lol'); 
            }
            // echo "<pre>"; print_r($resltip);  exit('lljjjjj'); 
            ...
            foreach ($res_ptype as $ptype){
                ...
                $query_pt =  "SELECT SUM(total_w_tax) , order_id, count(order_id) FROM mt_order WHERE `date_created` BETWEEN  '".$date_start ."'  AND  '". $date_end ."'  AND merchant_id = ".$arr."  AND status = 'paid'  AND  payment_type = '". $ptype ."'";  // this line new
                ...
                $i++; 
                         ...
                         // this line new
                    // $delivery_charges += $resltips['SUM(delivery_charge)'];
                    // $delivery_charges_count += $resltips['count(delivery_charges_count)']; 
                    // $tax +=  $resltips['taxable_total']; 


            $payment_with_type[$i]['count(order_id)'] = $tip_pay_count ;  // this line new
            $payment_with_type[$i]['SUM(total_w_tax)'] = $tip_pay;
            array_push($payment_with_type[$i], 'Payment Tips'); 

            $payment_with_type[$i+1]['SUM(total_w_tax)'] = $tax;
            array_push($payment_with_type[$i+1], 'Tax');

            $payment_with_type[$i+2]['count(order_id)'] =  $delivery_charges_count ;  // this line new
            $payment_with_type[$i+2]['SUM(total_w_tax)'] = $delivery_charges;
            array_push($payment_with_type[$i+2], 'Delivery Fee');    

               

            // echo "<pre>"; print_r($payment_with_type);  exit('mannnn'); 
            ...
            if($result){
                // echo "<pre>"; print_r($result); exit('mereeee');  
                return $result; 
            }    
        }
    }
    ...
    public function get_versiPOS_api_data($data = ''){
		if(is_array($data)){
			$currentDate = date('Ymd');
            $date_starts = ''; 
            $config = $this->get_versiPOS_api_configuration($data['restaurant_name_versiPos']);
        }
        else{
            $currentDate = '';
            $date_starts = ''; 
            $config = $this->get_versiPOS_api_configuration($data);
        }
		...
		if(isset($list->ErrorMesg))
		{
			$val['error'] =  '<center>'.$list->ErrorMesg .' Contact to admin </center> ' ;
			$val['list'] = false;
			return $val; 
		}	
        $val['list'] = $list->Reports; 
      
        $report_checkboxes =  array(); 
        
		if(isset($data['checkboxes1'])){
            $report_checkboxes = array_merge($report_checkboxes, $data['checkboxes1']);
        } 
        if(isset($data['checkboxes2'])){
            $report_checkboxes = array_merge($report_checkboxes, $data['checkboxes2']);
        } 

        if(isset($data['api_day'])){
            if($data['api_day']  ==   1){
                $currentDate  = '';
                $date_starts = '';
            }
            if($data['api_day']  ==   2){
                $date_starts  = date('Ymd', strtotime("-1 day", strtotime(date('Ymd'))));
                $currentDate  = date('Ymd', strtotime("-1 day", strtotime(date('Ymd'))));
            } 
            elseif($data['api_day'] == 3){
                $date_starts  = date('Ymd', strtotime("-7 day", strtotime(date('Ymd'))));
            }  
            elseif($data['api_day'] == 4){
                $date_starts  = date('Ymd', strtotime("-30 day", strtotime(date('Ymd'))));
            }
            elseif($data['api_day'] == 5){
                $date_starts  =  date('Ym'). '01';
            }
            elseif($data['api_day'] == 6){
                $monthforlast  = date('M');
                $month1  = date('m');
                $month  = $month1-1;    
                if(strlen((string)$month) == 1){
                    $date_starts  = date('Y').'0'.$month.'01';
                }
                else{
                    $date_starts  = date('Y').''.$month.'01';
                }   
				$currentDate  = date('Ymt',strtotime($date_starts));
            }  
            elseif($data['api_day'] == 7){
                $date_starts  = date('Ymd', strtotime($data['datefrom']));
                $currentDate  = date('Ymd', strtotime($data['datefrom']));
            }   
             elseif($data['api_day'] == 8){
                $date_starts  = date('Ymd', strtotime($data['datefrom']));
                $currentDate  = date('Ymd', strtotime($data['dateto']));
            }   
        }
        if(isset($data['api_day'])){
           
            if(!empty($report_checkboxes)){
                $im_report_checkboxes = implode(',', $report_checkboxes); 
        ...
        if($reports_fields){
            //$ch = curl_init('https://api.versipos.net:1502/dataconnect/GET_REPORT');
			$ch = curl_init('https://reports.versipos.net:1502/dataconnect/GET_REPORT');
             // $reports_fields = http_build_query($reports_fields);
            curl_setopt_array($ch, array(
            CURLOPT_POST => TRUE,
            CURLOPT_RETURNTRANSFER => TRUE,
            CURLOPT_SSL_VERIFYPEER => false,
            ...
            CURLOPT_POSTFIELDS => json_encode($reports_fields)
            ));
            
            $response = curl_exec($ch);
            curl_close($ch);
			$rep = json_decode($response);
			//echo "<pre>"; print_r($rep); exit('outt');  
			if(isset($rep->ErrorMesg))
			{
				$val['error'] =  '<center>'.$rep->ErrorMesg .' Contact to admin </center> ' ;
				return $val; 
				
			}
			else{
				$val['ReportTxt'] = $rep->Reports;
			}
            
			$val['Report_details'][0] = trim($rep->ReportHeader[0]);
            $val['Report_details'][1] = trim($rep->ReportHeader[1]); 
            $val['Report_details'][2] = trim($rep->ReportHeader[2]);    

            return  $val;            
            }
        else{
            $default_report_checkboxes = $this->versiPos_reports_default_filter('', '',$data)->filters;
            
			$default_report_checkboxes = json_decode($default_report_checkboxes);
            
			// echo "<pre>"; print_r($default_report_checkboxes); exit('ji'); 
            // $im_report_checkboxes = implode(',', $report_checkboxes); 
                
                $reports_fields_loading= array(
                "ApiKey"        => "d17aaf42f76047339de1c749e9cf3bbc",
                "MerchantID"    => $config['versiPOS_merchant_id'],
                "StoreID"       => $config['versiPOS_store_id'],
                "Username"      => $config['versiPOS_username'],
                ...
                "Start"         => $date_starts,
                "End"           => $currentDate,
                "ReportID"      => $default_report_checkboxes //'[' . $im_report_checkboxes. ']'
                );
			//echo "<pre>"; print_r(json_encode($reports_fields_loading)); exit('kkk');  
			//echo "<pre>"; print_r($reports_fields_loading); exit('ji'); 
            ...
            $ch = curl_init('https://reports.versipos.net:1502/dataconnect/GET_REPORT');  
                ...
                CURLOPT_POSTFIELDS => json_encode($reports_fields_loading)
                ));
                $response = curl_exec($ch);
                $repor = json_decode($response);
           //echo "<pre>"; print_r($repor) ; exit('jillllhhhhhlout');      
			//echo "<pre>"; print_r($repor->ReportHeader[0]); exit('kkk'); 
                $val['Report_details'][0] = trim($repor->ReportHeader[0]);
                $val['Report_details'][1] = trim($repor->ReportHeader[1]); 
                $val['Report_details'][2] = trim($repor->ReportHeader[2]);    
            }

        $val['ReportTxt'] = $repor->Reports;
        // echo "<pre>"; print_r($val) ; exit('jillllhhhhhlout'); 
        return  $val; 
        // //close connection
        
    }

        public function get_versiPos_merchants($id = '', $list =''){
           ...
           foreach ($result as $res) {
              if($res['versiPOS_username'] !='' && $res['versiPOS_password'] !='' && $res['versiPOS_merchant_id'] !='' && $res['versiPOS_store_id'] !=''){
				  ...
				  //$date_starts = ''; $currentDate = ''; 
				  //if(!empty($list)){
					  //$default_report_checkboxes[0] = $list[0]->ReportID; 
					//  $default_report_checkboxes[0] = 0; 
				  //}
				  
				 // $fields = array(
                     //   "ApiKey"=> "d17aaf42f76047339de1c749e9cf3bbc",
                     //   "MerchantID"	=> $res['versiPOS_merchant_id'],
                     //   "StoreID"   	=> $res['versiPOS_store_id'],
                     //   "Username"  	=> $res['versiPOS_username'],
                     //   "Password"  	=> $res['versiPOS_password'],
                     //   "Start"         => $date_starts,
					//	"End"           => $currentDate,
					//	"ReportID"      => $default_report_checkboxes
                      //  );
				    //echo "<pre>"; print_r(json_encode($fields)) ; exit('hello'); 
                   ...
                   // ),
                    ...
                    //));
                    //$response = curl_exec($ch);
                    //$com_details = json_decode($response);
				  	//if(isset($com_details->ErrorMesg))
					//	{
					//		$error =  '<center>'.$com_details->ErrorMesg .' Contact to admin </center> ' ;
					//		echo $error; exit();
					//	}
				  //$api_result = trim($com_details->ReportHeader[0]);  
                    // $api_result = $this->get_versiPOS_api_data($res['userid']); 
                    
                    //$merchant_name_versiPos = explode(':', $api_result);
                    //if($api_result){
                        array_push($api_company, array('client_merchant_versiPos' =>$res['userid'], 'merchant_name_versiPos' => $res['company']));      
                    //}
                }
           }
	      if($api_company){  
   ...
   public function get_customers_lead($id = '', $where = array())
    {
		$this->db->select('tblcustomerleads.*, tblleadsstatus.name as status_name, tblleadssources.name as source_name');
		$this->db->from('tblcustomerleads');
		$this->db->join('tblleadsstatus', 'tblcustomerleads.status=tblleadsstatus.id', 'left');
		$this->db->join('tblleadssources', 'tblcustomerleads.source=tblleadssources.id', 'left');
		$this->db->where('tblcustomerleads.id', $id); 
		if($where){
			$this->db->where($where); 
		}
		return $this->db->get()->row();
		// $query = $this->db->query("SELECT tblcustomerleads.*, tblleadsstatus.name as status_name, tblleadssources.name as source_name FROM `tblcustomerleads` LEFT JOIN tblleadsstatus on tblcustomerleads.status = tblleadsstatus.id LEFT JOIN tblleadssources on tblcustomerleads.source = tblleadssources.id WHERE tblcustomerleads.id = $id");
		// $result = $query->row();
		// return $result;
    }
	
    public function versiPos_reports_default_filter($data = '', $action='', $selc = '' ){
            $this->db->select('*');
            $this->db->from('tblversipos_reports_filters');
            $this->db->where('contact_id', $this->session->userdata('contact_user_id'));
            if($selc){
                $this->db->where('client_id', $selc);
            }
            $filters = $this->db->get()->row();
        if($action == 'insert'){
            if($filters){
                // echo "<pre>";  print_r($data); exit('jijij');
                $this->db->where('contact_id',$this->session->userdata('contact_user_id'));
                $this->db->where('client_id',$selc);
                $this->db->update('tblversipos_reports_filters', $data);
            }
            else{
                $inserted = $this->db->insert('tblversipos_reports_filters', $data);
                return  $inserted;
            } 
        }
        if($action == ''){ 
    ...
    public function versieats_merchants_info($id ='', $select = '*'){
        $this->load_versieats_db();
         if($id){
            $query_time_zone = "SELECT " . $select . " FROM `mt_merchant` WHERE `merchant_id` = " . $id ; 
            $qtime_zone = $this->versi_con->query($query_time_zone)->result_array();        
         }
         else{
            $query_time_zone = "SELECT " . $select . " FROM `mt_merchant` "; 
            $qtime_zone = $this->versi_con->query($query_time_zone)->result_array();        
         }
         return $qtime_zone[0]; 
    }
    public function versieats_settings_save($data = '', $id =''){
    
    $this->load_versieats_db();

    if($data['merchants_turn']){
        $merchants_turn = $data['merchants_turn'];
        unset($data['merchants_turn']);  
    }
     if($data['merchants']){
        $merchant_id = $data['merchants'];
        unset($data['merchants']);  
    }

     if($data['merchant_holiday']){
        $merchant_holiday = $data['merchant_holiday'];
        unset($data['merchant_holiday']);  
    }
    
    $i = 0; 
    
    $this->versi_con->where('merchant_id',$merchant_id);
    $insert1 = $this->versi_con->update('mt_merchant', $data);
    if($insert1){
        $i++;
    }

    $this->versi_con->where('merchant_id',$merchant_id);
    $this->versi_con->where('option_name','merchant_close_store');
    $insert2= $this->versi_con->update('mt_option', array('option_value'=> $merchants_turn) );
    if($insert2){
        $i++;
    }

    $this->versi_con->where('merchant_id',$merchant_id);
    $this->versi_con->where('option_name','merchant_holiday');
    $insert3= $this->versi_con->update('mt_option', array('option_value'=> $merchant_holiday) );
    if($insert3){
        $i++;
    }
    
    // $post_data['merchant_holiday']
    return $i;
    ...
    {
        $this->load_versieats_db();
        $this->versi_con->select('service,turn_off_pickup,turn_off_delivery,dp_interval,dd_interval,temp_pickup_interval,temp_pickup_next_time,temp_delivery_interval,temp_delivery_next_time');
        $this->versi_con->from('mt_merchant');
        $this->versi_con->where('merchant_id',$id);
        $result1 = $this->versi_con->get()->result_array();

        $this->versi_con->select('option_value');
        $this->versi_con->from('mt_option');
        $this->versi_con->where('option_name','merchant_close_store');
        $this->versi_con->where('merchant_id',$id); 
        $result2 = $this->versi_con->get()->result_array();

        $this->versi_con->select('option_value');
        $this->versi_con->from('mt_option');
        $this->versi_con->where('option_name','merchant_holiday');
        $this->versi_con->where('merchant_id',$id); 
        $result3 = $this->versi_con->get()->result_array();
        // print_r($result3); exit('lkklklklklk');  
