...
class Misc_model extends CRM_Model
{
    ...
    {
		$client = $this->db->where('id', $rel_id)->get('tblleads')->row();
		if(!empty($client)) {
		...
		if($rel_type == 'lead' && $clientId) {
			 $this->db->or_where('rel_id', $clientId);
		} else {
    ...
    public function perform_search($q)
    {
		...
		if ($have_assigned_customers || $have_permission_customers_view) {			 		
			...
			$this->db->join('tblcontacts_rel_clients', 'tblcontacts_rel_clients.company_id = tblclients.userid AND tblcontacts_rel_clients.is_primary = 1 ', 'left');
        ...
        $surveys_search = $this->_search_surveys($q, $limit);
        if (count($surveys_search['result']) > 0) {
            $result[] = $surveys_search;
        }
       $knowledge_base_search = $this->_search_knowledge_base($q, $limit);
        ...
        //     $this->db->select()->from('tblsurveys')->where('tblsurveys.portfolio_id', $this->session->userdata("portfolio_id"))->like('subject', $q)->or_like('slug',$q)->or_like('description', $q)->or_like('viewdescription', $q )->limit($limit);
        //     // print_r($e); exit; 
		...
		$this->db->where('tblstafftasks.portfolio_id', $this->session->userdata("portfolio_id"));
        if (!$is_admin) {
            if (!$tasks) {
        ...
        if (has_permission('goals', '', 'view')) {
            // Goals
            $this->db->select()->from('tblgoals');
			$this->db->where('tblgoals.portfolio_id', $this->session->userdata("portfolio_id"));
			$this->db->like('description', $q)->or_like('subject', $q)->limit($limit);
    ...
    public function _search_proposals($q, $limit = 0)
    {
        ...
        if ($has_permission_view_proposals || $has_permission_view_proposals_own) {
			...
			$this->db->where('tblproposals.portfolio_id', $this->session->userdata("portfolio_id"));
    ...
    {
        $where['portfolio_id'] = $this->session->userdata("portfolio_id"); 
        $result = array(
            'result' => array(),
            'type' => 'leads',
    ...
    public function _search_tickets($q, $limit = 0)
    {
        ...
        if (is_staff_member() || (!is_staff_member() && get_option('access_tickets_to_none_staff_members') == 1)) {
            ...
            // check this is versitech portfoio or not 
            $check = 0;
            if(!null == $this->session->userdata('portfolio_id')){
              $check = main_portfolio($this->session->userdata('portfolio_id'));  
            }
			...
			$this->db->join('tblcontacts_rel_clients', 'tblcontacts_rel_clients.contact_id = tbltickets.contactid ', 'left');
    ...
    public function _search_contacts($q, $limit = 0, $where = '')
    {
        ...
        if ($have_assigned_customers || $have_permission_customers_view) {
            // Contacts
            $this->db->select(implode(',', prefixed_table_fields_array('tblcontacts')).', tblcontacts_rel_clients.company_id, tc.company as company');
            $this->db->from('tblcontacts');
            $this->db->join('tblclients', 'tblclients.userid=tblcontacts.userid', 'left');
			$this->db->join('tblcontacts_rel_clients', 'tblcontacts_rel_clients.contact_id=tblcontacts.id', 'left'); //New added by zee
			$this->db->join('tblclients as tc', 'tc.userid=tblcontacts_rel_clients.company_id', 'left'); //New added by zee

            // $this->db->where('tblcontacts.portfolio_id', $this->session->userdata("portfolio_id"));
            if(empty($check))
                {
                   $this->db->where('tblcontacts.portfolio_id', $this->session->userdata("portfolio_id"));
                }
			...
			$this->db->group_by('tblcontacts.id');
            if ($limit != 0) {
                $this->db->limit($limit);
            }
    ...
    public function _search_contracts($q, $limit = 0)
    {
        ...
        if ($has_permission_view_contracts || has_permission('contracts', '', 'view_own')) {
            ...
            $this->db->where('tblcontracts.portfolio_id', $this->session->userdata("portfolio_id"));
    ...
    public function _search_surveys($q, $limit = 0)
    {
            $result = array(
                'result' => array(),
                'type' => 'surveys',
            ...
            if (has_permission('surveys', '', 'view')) { 
                $this->db->select();
                $this->db->from('tblsurveys');
                $this->db->where('tblsurveys.portfolio_id', $this->session->userdata("portfolio_id"));
                $this->db->where('(subject LIKE "%' . $q . '%" OR slug LIKE "%' . $q . '%" OR description LIKE "%' . $q . '%" OR viewdescription LIKE "%' . $q . '%")');
                if ($limit != 0) {
                    $this->db->limit($limit);
                }
                $this->db->order_by('subject', 'ASC');
     ...
     public function _search_knowledge_base($q, $limit = 0)
    {
            ...
            if (has_permission('knowledge_base', '', 'view')) { 
                ...
                $this->db->where('tblknowledgebase.portfolio_id', $this->session->userdata("portfolio_id"));
                $this->db->where('(subject LIKE "%' . $q . '%" OR description LIKE "%' . $q . '%" OR slug LIKE "%' . $q . '%" )');
                if ($limit != 0) {
                    $this->db->limit($limit);
                }
                $this->db->order_by('subject', 'ASC');
    ...
    public function _search_projects($q, $limit = 0, $where = false)
    {
		...
		$this->db->where('tblprojects.portfolio_id', $this->session->userdata("portfolio_id"));
        if (!$projects) 
		{
	...
	//bilal code start
	public function _search_projects_so($q, $limit = 0, $where = false)
    {
        $result = array(
            'result' => array(),
            'type' => 'saleopp',
            'search_heading' => _l('saleopp')
        );

        $projects = has_permission('sales', '', 'view');
        // Projects
        $this->db->select();
        $this->db->from('tblsales');
        $this->db->join('tblclients', 'tblclients.userid = tblsales.clientid');
		$this->db->where('tblsales.portfolio_id', $this->session->userdata("portfolio_id"));
        if (!$projects) {
            $this->db->where('tblsales.id IN (SELECT project_id FROM tblsalemembers WHERE staff_id=' . get_staff_user_id() . ')');
        }
        if($where != false){
            $this->db->where($where);
        }
        if (!_startsWith($q, '#')) {
            ...
            $this->db->where('id IN
                (SELECT rel_id FROM tbltags_in WHERE tag_id IN
                (SELECT id FROM tbltags WHERE name="' . strafter($q, '#') . '")
                AND tbltags_in.rel_type=\'saleopp\' GROUP BY rel_id HAVING COUNT(tag_id) = 1)
                ');
        }
    ...
    public function _search_invoices($q, $limit = 0)
    {
        ...
        if ($has_permission_view_invoices || $has_permission_view_invoices_own) {
			...
			$this->db->join('tblcontacts_rel_clients', 'tblcontacts_rel_clients.company_id=tblclients.userid AND is_primary = 1', 'left'); //New added by zee	
            $this->db->where('tblinvoices.portfolio_id', $this->session->userdata("portfolio_id"));
			if (!$has_permission_view_invoices) {
                $this->db->where('addedfrom', get_staff_user_id());
            }
             if (!_startsWith($q, '#')) {
    ...
    public function _search_estimates($q, $limit = 0)
    {
        ...
        if ($has_permission_view_estimates || $has_permission_view_estimates_own) {
            ...
            $this->db->where('tblestimates.portfolio_id', $this->session->userdata("portfolio_id"));  
    ...
    public function _search_expenses($q, $limit = 0)
    {
        ...
        if ($has_permission_expenses_view || $has_permission_expenses_view_own) {
            ...
            $this->db->where('tblexpenses.portfolio_id', $this->session->userdata("portfolio_id"));  
	...
	public function get_reminder_fields($id)
	{
		$this->db->select("*");
	...
	// public function param_value($param1 , $param2)
	 // {	//echo $param1 ;
		...
		// return $data;
	// }
    public function get_company_reminders($id = ''){
        if (is_numeric($id)) {
            $this->db->where('tblreminders.rel_id', $id);
        }
        $this->db->order_by('date', 'desc');
