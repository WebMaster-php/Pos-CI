...
class Proposals_model extends CRM_Model
{
    ...
    public function add($data)
    {
        ...
        if ($insert_id) {
            ...
            if ($save_and_send === true) {
				...
				$check_proposal = $this->get_contactid_($insert_id);
				$conid = $check_proposal[0]['contact_id']; 
				
				$check_proposal_value = $this->rad_check($conid, 'email_alert_proposal');
				
				if($check_proposal_value[0]['value']==1 || empty($check_proposal_value) )
				{	
	...
	public function sms_proposal($id)
	{
		$this->db->select('*');
		$this->db->from('tblproposals');
		$this->db->where('id', $id);
		...
		$company_id = $res[0]['rel_id'];
		
		$this->db->select('tblcontacts_rel_clients.*, tblcontacts.*');
		$this->db->from('tblcontacts_rel_clients');
		$this->db->join('tblcontacts', 'tblcontacts.id = tblcontacts_rel_clients.contact_id', 'left');
		$this->db->where('tblcontacts_rel_clients.company_id', $company_id);
		$this->db->where('tblcontacts_rel_clients.is_primary', 1);		
		$c = $this->db->get()->result_array();
 		$conid = $c[0]['contact_id'];
		
		$this->db->select('*');
		$this->db->from('tblrad');
		$this->db->where('userid', $conid);
		$this->db->where('name', 'sms_alert_proposal');
		...
		{	
			if($c[0]['phonenumber'])
			{
				if(substr($c[0]['phonenumber'] , 0, 1) == '+')
					{
						$number = $c[0]['phonenumber'];
					}
				else
					{
						$number = '+'.$c[0]['phonenumber'];
				...
				$msg = 'Hello '. $res[0]['proposal_to']. ', a new proposal has been sent, you can view it here:  '. base_url('clients/proposals') ; 
	...
	public function get_contactid_($id)
	{
		$this->db->select('*');
		$this->db->from('tblproposals');
		$this->db->where('id', $id);
