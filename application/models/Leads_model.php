...
class Leads_model extends CRM_Model
{
    ...
    public function add($data)
    {
        ...
        if ($insert_id) {
            ...
            $this->lead_assigned_member_notification($insert_id, $data['followers']);
	...
	public function updateCustomerLeads($data, $id)
    {
        $current_lead_data = $this->get($id);
        $current_status    = $this->get_status($current_lead_data->status);
        if ($current_status) {
	...
	// public function add_followers($data){
	 ...
	 // $isFollowerAlready=$this->db->get('tblfollowers')->result();
	 // if($isFollowerAlready){
		 // return false;
	 // }else{
		 // $this->db->insert('tblfollowers',$data);
		 // return $this->db->insert_id();
	 // }
	// }
	// public function edit_followers($data){
		// $this->db->where('id',$data['id']);
		// $this->db->update('tblfollowers',$data);
		// return true;
	// }
	// public function delete_follower($id){
		// $this->db->where('id', $id);
        // $this->db->delete('tblfollowers');
		// return true;
	// }
	
	// public function get_followers(){
		// return $this->db->get('tblfollowers')->result();
	// }
	
	public function getLeadDetailsById($id){
		return $this->db->where('id',$id)->get('tblleads')->row();
	}
	public function getTableRecordById($table,$where){

		return $this->db->where($where)->get($table)->row();
		
	}
    public function getTableRecords($table,$where){

        return $this->db->where($where)->get($table)->result();
        
    }
    public function add_contacts_to_customer($data){
          return $this->db->insert('tblcontacts_rel_clients', $data);
    ...
    public function add_contact($data, $customer_id, $not_manual_request = false)
    {
        ...
        $data['lead_email'] = trim($data['lead_email']);
        ...
        {   if($is_primary ==1){
                ...
                $this->db->update('tbl_lead_contacts_rel_clients', array('is_primary'=>0));
            } 
            ...
            $this->db->insert('tbl_lead_contacts_rel_clients', $query);
        ...
        {    
            ...
            $this->db->insert('tblleadcontacts', $data);
        ...
        if ($contact_id) {
            ...
            if ($not_manual_request == true) {
                ...
                $this->db->update('tblleadcontacts', array(
                    'invoice_emails'=>0,
                    'estimate_emails'=>0,
            ...
            foreach ($permissions as $permission) {
                ...
                if ($not_manual_request == true) {
                    if ($permission == 6) {
                        $this->db->where('id', $contact_id);
                        $this->db->update('tblleadcontacts', array('project_emails'=>1, 'task_emails'=>1));
                    } elseif ($permission == 3) {
                        $this->db->where('id', $contact_id);
                        $this->db->update('tblleadcontacts', array('contract_emails'=>1));
                    } elseif ($permission == 2) {
                        $this->db->where('id', $contact_id);
                        $this->db->update('tblleadcontacts', array('estimate_emails'=>1));
                    } elseif ($permission == 1) {
                        $this->db->where('id', $contact_id);
                        $this->db->update('tblleadcontacts', array('invoice_emails'=>1, 'credit_note_emails'=>1));
                    }
                }
            }
            ...
            if ($send_welcome_email == true) {
                ...
                $this->emails_model->send_email_template('new-client-created', $data['lead_email'], $merge_fields);
            }

            if ($send_set_password_email) {
                $this->authentication_model->set_password_email($data['lead_email'], 0);
            }

            logActivity('Contact Created [' . $data['first_name'] . ' ' . $data['last_name'] . ']');
    ...
    public function get_contact($id)
    {
        ...
        return $this->db->get('tblleadcontacts')->row();
    }
    ...
    {   
        ...
        $this->db->where('company_id', $company_id);
        $this->db->where('contact_id', $id);
        $this->db->update('tbl_lead_contacts_rel_clients', $query);
        if ($this->db->affected_rows() > 0) 
        {   
            $affectedRows++;
        }       
        ...
        $data = $hook_data['data']; 
        $this->db->where('id', $id);
        $this->db->update('tblleadcontacts', $data);    
        ...
        if ($client_request == false) {
            ...
            if ($send_set_password_email) {
                $set_password_email_sent = $this->authentication_model->set_password_email($data['lead_email'], 0);
            }
        }
...
/**
    ...
    { 
        $this->db->select('tblleadcontacts.*, cs1.*');
        $this->db->from('tblleadcontacts');
        $this->db->join('tbl_lead_contacts_rel_clients as cs1', 'cs1.contact_id = tblleadcontacts.id', 'LEFT');
        $this->db->where($where);
    ...
    public function delete_contacts_rel_clients($company_id, $contact_id)
    {
        ...
        $this->db->where("company_id", $company_id);
        $query = $this->db->delete('tbl_lead_contacts_rel_clients');
        if ($query)
        { 
            return true;
        }
    ...
    {    
        $this->db->select("*"); 
        $this->db->from("tbl_lead_contacts_rel_clients"); 
        $this->db->where('company_id', $company_id );
        $this->db->where('contact_id', $contact_id ); 
	...
	public function getAllTableRecord($table){
        $this->db->where('portfolio_id' , $this->session->userdata('portfolio_id'));
        $as = $this->db->get($table)->result();
        return $as; 
    }
    public function get_customer_record($userid){
        $this->db->where('userid',$userid);
        $this->db->where('portfolio_id' , $this->session->userdata('portfolio_id'));
       $customer =  $this->db->get('tblclients')->row();
       
       // $customer=$this->db->where('userid',$userid)->where('portfolio_id' , $this->session->userdata('portfolio_id'))->get('tblclients')->row();
        if($customer){
           	$already_lead=$this->db->where(['name' => $customer->company, 'client_id' => $customer->userid])->get('tblleads')->row();
	...
	public function delete_contact($company_id, $contact_id)
    {
        $this->db->where("contact_id", $contact_id);
        $this->db->where("company_id", $company_id);
