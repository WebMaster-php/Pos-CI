...
class Staff_model extends CRM_Model
{
   ...
   public function get_ticket_staff($id = '', $active = '', $where = array())
    {
    ...
    public function get_ticket_staff_multi_portfolio($id = '', $active = '', $where = array())
    {
        $check = main_portfolio($this->session->userdata('portfolio_id'));
        ...
        if($check == ''){
            // $this->db->where('portfolio_id ', 1);
            $values = array();
            array_push($values, 1);
            array_push($values, $this->session->userdata('portfolio_id'));
            $this->db->where_in('portfolio_id', $values);
        }
        
        $this->db->order_by('firstname', 'desc');
        // $this->db->get('tblstaff')->result_array();
    ...
    {		
        ...
        if (!isset($data['tbl_staff_portfolio'])) {
             $data['tbl_staff_portfolio'] = 0;
            }

        if ($data['admin'] == 1) {
            $data['is_not_staff'] = 0;
        }
        if (!isset($data['hidden_items'])) {
            $data['hidden_items'] = 0;
        }
        if (isset($data['hidden_items']) && $data['hidden_items'] ==1) {
            $data['hidden_items'] = 1;
        }
         if (!isset($data['email_notify_enable'])) {
            $data['email_notify_enable'] = 0;
        }

        if(isset($data['inventory_status']))
        {
            $data['inventory_status'] = array_filter($data['inventory_status']); 
            $data['inventory_status'] = implode(',',$data['inventory_status']);
        }
      ...
      // echo "<pre>"; print_r($data); exit; 
        ...
        if ($staffid) {
			//here my code 1 oct start
			$this->portfolio($portfolio, $staffid);
            ...
            //echo "<pre>";  print_r($_all_permissions); exit; 
    ...
    public function update($data, $id)
    {
        ...
        if(isset($data['inventory_status']))
        {
            // echo "<pre>"; print_r($data['inventory_status']); exit('inni');
            $data['inventory_status'] = array_filter($data['inventory_status']); 
            // echo "<pre>"; print_r(array_filter($data['inventory_status'])); exit('inni');
        ...
        $this->db->where('staffid', $id);
        $ds = $this->db->update('tblstaff', $data);
// echo $ds; 
// 		echo "<pre>"; print_r($data);exit('jjijiij');   
        //here my code 
        $this->update_portfoilio($portfolio, $id);
		// here my code end 
		if ($this->db->affected_rows() > 0) {
            $affectedRows++;
	...
	public function portfolio($portfolio, $addedfor)
	{
		$data = array(
			'portfolio_name'=> json_encode($portfolio),
			'addedfor'		=> $addedfor,
			'addedby'		=> $this->session->userdata('staff_user_id'),
			'createdat'			=> date('Y-m-d H:i:s')
		
		);
		$this->db->insert('tblportfolio', $data);
	...
	public function get_portfolio($id)
	{
		...
		$decode = json_decode($result[0]['portfolio_name']);
		$p_name = array();
		...
		foreach($decode as $de)
			...
			$this->db->from('tblportfolio_names');
			$this->db->where('name_id', $de);
	...
	public function get_portfolio_check($id)
	{
		$this->db->select('portfolio_name');
		$this->db->from('tblportfolio');
		$this->db->where('addedfor', $id);
	...
	public function update_portfoilio($data, $id)
	{
		...
		if($res)
			{ 
				$data = array('portfolio_name'=>json_encode($data));
				$this->db->where('addedfor', $id);
				$da = $this->db->update('tblportfolio', $data);
			}
		
		else
		{	
				$data = array(
				'portfolio_name'=> json_encode($data),
				'addedfor'		=> $id,
				'addedby'		=> $this->session->userdata('staff_user_id')
	...
	{
		$this->db->select('tblportfolio.* ');
			$this->db->from('tblportfolio');
			$this->db->where('addedfor', $id);
			$res = $this->db->get()->result_array();
			$ed = $res[0]['portfolio_name'];
			$ar = json_decode($ed);
			$array = array();
			foreach($ar as $a)
			{
				...
				$this->db->where('id', $a);
				$res = $this->db->get()->row();
				$re = json_decode($res->modules);			
				
				while ($fruit_name = current($re)) 
				{
					$a =  key($re);
	...
	public function staff_setting($data){
            if(!isset($data['notify_inventory'])){
                $data['notify_inventory'] = 0; 
            }

            if(isset($data['ticket_permissions'])){
                $data['ticket_permissions'] = serialize($data['ticket_permissions']);; 
            }
            if(!isset($data['ticket_permissions'])){
            ...
            if(isset($data['inventory_status'])){
                $data['inventory_status'] = array_filter($data['inventory_status']);
                $val = implode(',', $data['inventory_status'] );
                $val = rtrim($val, ',');
                $data['inventory_status'] = $val; 
            }
            $this->db->where('staffid' , $data['staffid']);
            $upd = $this->db->update('tblstaff', $data);  
            if($upd){
                return $upd; 
            }  
    }
