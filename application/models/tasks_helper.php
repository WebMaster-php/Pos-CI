...
function tasks_rel_name_select_query()
{
    return '(CASE rel_type
        ...
        WHEN "customer" THEN (SELECT CASE company WHEN "" THEN (SELECT CONCAT(firstname, " ", lastname) FROM tblcontacts JOIN tblcontacts_rel_clients ON tblcontacts.id = tblcontacts_rel_clients.contact_id WHERE tblcontacts.userid = tblclients.userid and tblcontacts_rel_clients.is_primary = 1) ELSE company END FROM tblclients WHERE tblclients.userid=tblstafftasks.rel_id)
        WHEN "project" THEN (SELECT CONCAT(CONCAT(CONCAT("#",tblprojects.id)," - ",tblprojects.name), " - ", (SELECT CASE company WHEN "" THEN (SELECT CONCAT(firstname, " ", lastname) FROM tblcontacts JOIN tblcontacts_rel_clients ON tblcontacts.id = tblcontacts_rel_clients.contact_id WHERE userid = tblclients.userid and tblcontacts_rel_clients.is_primary = 1) ELSE company END FROM tblclients WHERE userid=tblprojects.clientid)) FROM tblprojects WHERE tblprojects.id=tblstafftasks.rel_id)
...
function init_relation_tasks_table($table_attributes = array())
{
	...
	//bilal code start
	    if ($table_attributes['data-new-rel-type'] == 'saleopp') {
    ...
    if (has_permission('tasks', '', 'create')) {
        ...
        if ($table_attributes['data-new-rel-type'] == 'customer' && is_numeric($table_attributes['data-new-rel-id'])) {
			...
			//print_r($table_data);
			//exit;
			//echo $table_attributes['data-new-rel-type']; exit;
        // projects have button on top
        if ($table_attributes['data-new-rel-type'] != 'project' && $table_attributes['data-new-rel-type'] != 'saleopp') {
	...
	//bilal code start
	else if ($table_attributes['data-new-rel-type'] == 'saleopp') {
        echo "<a href='" . admin_url('tasks/detailed_overview?saleopp=' . $table_attributes['data-new-rel-id']) . "' class='btn btn-success pull-right mbot25'>" . _l('detailed_overview') . "</a>";
          echo "<a href='" . admin_url('tasks/list_tasks?saleopp=' . $table_attributes['data-new-rel-id'] . '&kanban=true') . "' class='btn btn-default pull-right mbot25 mright5 hidden-xs'>" . _l('view_kanban') . "</a>";
        echo '<div class="clearfix"></div>';
        echo $CI->load->view('admin/tasks/_bulk_actions', array('table'=>'.table-rel-tasks'), true);
        echo $CI->load->view('admin/tasks/_summary', array('rel_id'=>$table_attributes['data-new-rel-id'], 'rel_type'=>'saleopp', 'table'=>$table_name), true);
        echo '<a href="#" data-toggle="modal" data-target="#tasks_bulk_actions" class="hide bulk-actions-btn table-btn" data-table=".table-rel-tasks">'._l('bulk_actions').'</a>';
    }
	//bilal code end
...
function tasks_summary_data($rel_id = null, $rel_type = null)
{
    ...
    foreach ($statuses as $status) {
        ...
        $tasks_my_where = 'id IN(SELECT taskid FROM tblstafftaskassignees WHERE staffid='.get_staff_user_id().') AND status='.$status['id'] . ' AND portfolio_id = '. $CI->session->userdata('portfolio_id') ;
        if ($rel_id && $rel_type) {
        ...
        $summary['total_tasks'] = total_rows('tblstafftasks', $tasks_where . ' AND tblstafftasks.portfolio_id =  ' . $CI->session->userdata('portfolio_id') );
        $summary['total_my_tasks'] = total_rows('tblstafftasks', $tasks_my_where . ' AND tblstafftasks.portfolio_id =  ' . $CI->session->userdata('portfolio_id') );
