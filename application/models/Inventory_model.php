...
class Inventory_model extends CRM_Model
{
   ...
   public function add($data)
    {	
        ...
        // $this->email_inventory_status(123, $data);
        $this->db->insert('tblinventoryadd', $data); //echo $this->db->last_query();exit;
		//$this->email_inventory_status($insert_id, $data);
    ...
    public function email_inventory_status($insert_id = '', $data = '')
    {
        $val_staff = $this->db->query("SELECT * FROM `tblstaff` WHERE `notify_inventory` = 1 AND `inventory_status` LIKE '%".$data['status']."%' ")->result_array();
        // echo "<pre>"; print_r($data); exit('jjiji');  
        if($val_staff){
            $i = 0; $s = array();
            foreach ($val_staff as $val) {
                    $this->db->select('value');
                    $this->db->from('tblcustom');
                    $this->db->where('id', $data['type_of_hardware']);
                    $type_of_hardware= $this->db->get()->row()->value;

                    $this->db->select('value');
                    $this->db->from('tblcustom');
                    $this->db->where('id', $data['status']);
                    $status_name= $this->db->get()->row()->value;
                    
                    $account_name = $this->inventory_model->get('tblclients',false,array('userid'=>$data['account']),'company');                
                    $message = "<html>
                        <head>
                        <title>Notification Email</title>
                        </head>
                        <body>
                        <p>Hi ".$val['firstname']." ".$val['lastname'].",</p>
                        <p>An inventory item has been marked as ' " .$status_name. " ' to the Account ". get_company_name($data['account']). " on " .date('m/d/Y',strtotime($data['current_date'])).".</p>
                        <p>The inventory item is: ".$type_of_hardware.", Serial number: ".$data['serial_number'].".</p>
                        <p>You can view the inventory item with the following link: <a href = '"  . base_url().'admin/inventory_items/notes/'.$insert_id . "' >'".$insert_id."': '".$account_name[0]['company']."'</a> </p>
                        <p>Kind Regards,</p>
                        <p>VersiPOS Team</p>
                        </body>
                        </html>";
						...
						//$email = 'zeeshananweraziz@gmail.com';		
                         // echo "<pre>"; print_r($message); exit('kokokokokokokokokokok');  
    ...
    {	
        ...
        {
			$this->email_inventory_status($where, $data);
			if (isset($custom_fields)) {
    ...
    { 
        ...
        // $this->db->where('cl.portfolio_id', $this->session->userdata('portfolio_id'));
        return $this->db->get()->result_array();
    }
	...
	{	
        ...
        // $this->db->where('tcc.portfolio_id', $this->session->userdata('portfolio_id'));
