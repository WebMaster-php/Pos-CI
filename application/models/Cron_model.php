...
class Cron_model extends CRM_Model
{
    ...
    {
        $checkDaily = get_option('daily_reminder_tasks_cron');
        //if($checkDaily==1){
            //$reminder_before = 1;
        //}else{
		...
		{ 
				if (date('Y-m-d', strtotime($task['duedate'])) > date('Y-m-d')) 
				{
					...
					if($task['daily_notofied_date'])
					{
						//$date_now = new DateTime();
						$date_now_dn = (new DateTime())->format('Y-m-d');
						$date_pd    = new DateTime($task['daily_notofied_date']);
						if ($task['daily_notofied_date'] && $date_now_dn > $date_pd) 
						{
							//echo 'greater than';
						}
						else{
				...
				if( ($checkDaily==1 && empty($task['daily_notofied_date']) ) || ( $checkDaily==1 && $date_now_dn > $date_pd ) )
				{
                    ...
                    foreach ($assignees as $member) {
                        ...
                        if ($row) {
                            ...
                            $this->db->update('tblstafftasks', array(
                                'deadline_notified' => 1,
                                'daily_notofied_date' => date('Y-m-d'),
                            ));
                        }
                    }
				}
                else if ($diff <= $reminder_before && $start_and_due_date_diff > $reminder_before) {
                    $assignees = $this->tasks_model->get_task_assignees($task['id']);
                    foreach ($assignees as $member) {
                        $this->db->select('email');
                        $this->db->where('staffid', $member['assigneeid']);
    ...
    public function check_leads_email_integration()
    {
        ...
        //echo "<pre>"; print_r($mail); exit('outer'); 
		if ($mail->active == 0) {
            return false;
        }
