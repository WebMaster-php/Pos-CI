...
class Sales_model extends CRM_Model
{
	...
	//bilal code start
	public function get_distinct_tasks_timesheets_staff_so($project_id)
    {
        return $this->db->query('SELECT DISTINCT staff_id FROM tbltaskstimers LEFT JOIN tblstafftasks ON tblstafftasks.id = tbltaskstimers.task_id WHERE rel_type="saleopp" AND rel_id=' . $project_id)->result_array();
    }
	//bilal code end
    ...
    public function timers_started_for_project($project_id, $where = array(), $task_timers_where = array())
    {
        ...
        $this->db->where('tblstafftasksso.rel_id', $project_id);
        $this->db->where('tblstafftasksso.rel_type', 'project');
        $this->db->join('tblstafftasksso', 'tblstafftasksso.id=tbltaskstimers.task_id');
    ...
    {
        if (total_rows('tblpinnedsales', array(
            'staff_id' => get_staff_user_id(),
            'project_id' => $id,
        )) == 0) {
            $this->db->insert('tblpinnedsales', array(
                'staff_id' => get_staff_user_id(),
                'project_id' => $id,
            ));
    ...
    public function calc_progress($id)
    {
        ...
        $this->db->where('id', $id);
        $project =  $this->db->get('tblsales')->row();
    ...
    public function get_last_project_settings()
    {
        ...
        $this->db->limit(1);
        $last_project = $this->db->get('tblsales')->row();
        if ($last_project) {
    ...
    public function get($id = '', $where = array())
    {
        ...
        if (is_numeric($id)) {
            $this->db->where('id', $id);
            $project = $this->db->get('tblsales')->row();//echo "<pre>"; print_r($project);exit;
            if ($project) { 
                $project->shared_vault_entries = $this->clients_model->get_vault_entries($project->clientid, array('share_in_sales'=>1));
                $settings          = $this->get_project_settings($id);

                // SYNC NEW TABS
                //$tabs = get_project_tabs_admin(null);
                $tabs = get_sales_opp_tabs_admin(null);
                $tabs_flatten = array();
                    ...
                    foreach ($tabs_flatten as $tab) {
                        if (!in_array($tab, $settings_available_features)) {
                            if ($available_features_index) {
                                ...
                                $this->db->update('tblsalesettings', array('value'=>serialize($tmp)));
                            }
                        }
                    }
                }
                ...
                foreach ($this->project_settings as $setting) {
                    if (!isset($project->settings->{$setting})) {
                        $this->db->insert('tblsalesettings', array(
                            'project_id' => $id,
                            'name' => $setting,
                            'value' => 0,
                        ));
        ...
        $this->db->join('tblclients', 'tblclients.userid=tblsales.clientid');
        $this->db->order_by('id', 'desc');
    ...
    public function get_tasks($id, $where = array(), $apply_restrictions = false, $count = false)
    {
        ...
        $select = implode(', ', prefixed_table_fields_array('tblstafftasks')).',tblmilestonesso.name as milestone_name,
        (SELECT SUM(CASE
           ...
           '.get_sql_select_task_assignees_ids_so().' as assignees_ids
        ';
        ...
        $this->db->join('tblmilestonesso', 'tblmilestonesso.id = tblstafftasks.milestone', 'left');
        $this->db->where('rel_id', $id);
        $this->db->where('rel_type', 'saleopp');
    ...
    public function get_files($project_id)
    {
        ...
        return $this->db->get('tblsalefiles')->result_array();
    }
    ...
    public function get_file($id, $project_id = false)
    {
        ...
        $this->db->where('id', $id);
        $file = $this->db->get('tblsalefiles')->row();
    ...
    public function update_file_data($data)
    {
        ...
        unset($data['id']);
        $this->db->update('tblsalefiles', $data);
    }

    public function change_file_visibility($id, $visible)
    {
        $this->db->where('id', $id);
        $this->db->update('tblsalefiles', array(
    ...
    public function remove_file($id)
    {
        ...
        if ($file) {
            if (empty($file->external)) {
                ...
                $path = get_upload_path_by_type_so('saleopp') . $file->project_id . '/';
            ...
            $this->db->where('id', $id);
            $this->db->delete('tblsalefiles');
            $this->log_activity($file->project_id, 'saleopp_activity_project_file_removed', $file->file_name, $file->visible_to_customer);
           
            // Delete discussion comments
            $this->_delete_discussion_comments($id, 'file');

           
            if (is_dir(get_upload_path_by_type_so('saleopp') . $file->project_id)) {
                // Check if no attachments left, so we can delete the folder also
                $other_attachments = list_files(get_upload_path_by_type_so('saleopp') . $file->project_id);
                if (count($other_attachments) == 0) {
                    delete_dir(get_upload_path_by_type_so('saleopp') . $file->project_id);
                }
            }
    ...
    public function get_project_overview_weekly_chart_data($id, $type = 'this_week')
    {
        ...
        $has_permission_create = has_permission('sales', '', 'create');
        // If don't have permission for sales create show only bileld time
        ...
        for ($i=0; $i<$loop_break; $i++) {
            ...
            $where = 'task_id IN (SELECT id FROM tblstafftasks WHERE rel_type = "saleopp" AND rel_id = "'.$id.'"';
    ...
    public function get_milestones($project_id)
    {
        $this->db->where('project_id', $project_id);
        $this->db->order_by('milestone_order', 'ASC');
        $milestones = $this->db->get('tblmilestonesso')->result_array();
        $i          = 0;
    ...
    public function add_milestone($data)
    {
        ...
        $this->db->insert('tblmilestonesso', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
            $this->db->where('id', $insert_id);
            $milestone = $this->db->get('tblmilestonesso')->row();
            ...
            $this->log_activity($milestone->project_id, 'saleopp_activity_created_milestone', $milestone->name, $show_to_customer);
            logActivity('Sale Opportunity Milestone Created [ID:' . $insert_id . ']');
    ...
    public function update_milestone($data, $id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update('tblmilestonesso', $data);
        if ($this->db->affected_rows() > 0) {
            ...
            $this->log_activity($milestone->project_id, 'saleopp_activity_updated_milestone', $milestone->name, $show_to_customer);
            logActivity('Sale Opportunity Milestone Updated [ID:' . $id . ']');
    ...
    public function update_milestone_color($data)
    {
        ...
        $this->db->update('tblmilestonesso', array(
            'color' => $data['color'],
        ));
    }
    ...
    public function delete_milestone($id)
    {
        ...
        $milestone = $this->db->get('tblmilestonesso')->row();
        $this->db->where('id', $id);
        $this->db->delete('tblmilestonesso');
        if ($this->db->affected_rows() > 0) {
            $project = $this->get($milestone->project_id);
            if ($project->settings->view_milestones == 1) {
                $show_to_customer = 1;
            ...
            $this->log_activity($milestone->project_id, 'saleopp_activity_deleted_milestone', $milestone->name, $show_to_customer);
            $this->db->where('milestone', $id);
            $this->db->update('tblstafftasks', array(
                'milestone' => 0,
            ));
            logActivity('Sales Opportunity Milestone Deleted [' . $id . ']');
	...
	//bilal code modified add_tasks($id, $product_id)
	public function add_tasks($id, $product_id)
    {
        $this->db->select('task_id');
        $this->db->where('product_id', $product_id);
        $tasks_ids = $this->db->get('tblproductstasksso')->result_array();
		// print_r($tasks_ids); die;
		for($i=0;$i<sizeof($tasks_ids); $i++)
		{
			$get_name =  $tasks_ids[$i]['task_id'];
			
			$this->db->select('name, startdate');
			$this->db->where('id', $get_name);
			$tasks_name = $this->db->get('tblstafftasks')->result_array();
			
			//get start date of project
			$this->db->select('start_date');
			$this->db->where('id', $id);
			$start_date = $this->db->get('tblsales')->result_array();
			
			// print_r($tasks_name); die;
			$que = $this->db->insert('tblstafftasks', 
				array(
					'rel_type' => 'saleopp',
					'rel_id' => $id,
					'name' => $tasks_name[0]['name'],
			...
			$insert_id = $this->db->insert_id();
			$datassign['assignee'] = get_staff_user_id();
			$datassign['taskid'] = $insert_id;
			$this->tasks_model->add_task_assignees($datassign);			
		}
    ...
    public function add($data)
    {
        ...
        if ($insert_id) {
            ...
            $this->log_activity($insert_id, 'saleopp_activity_created');
            ...
            logActivity('New Sale Opportunity Created [ID: ' . $insert_id . ']');
    ...
    public function update($data, $id)
    {
        ...
        $this->db->where('id', $id);
        $old_status = $this->db->get('tblsales')->row()->status;
        ...
        if (!isset($data['settings'])) {
            $this->db->where('project_id', $id);
            $this->db->update('tblsalesettings', array(
                'value' => 0,
            ));
            if ($this->db->affected_rows() > 0) {
                $affectedRows++;
        ...
        if ($affectedRows > 0) {
            ...
            if ($original_project->status != $data['status']) {
                ...
                if ($data['status'] == 4) {
                    ...
                    $this->db->where('id', $id);
                    $this->db->update('tblsales', array('date_finished'=>date('Y-m-d H:i:s')));
                } else {
    ...
    public function send_project_customer_email($id, $template)
    {
        ...
        $this->db->where('id', $id);
        $clientid = $this->db->get('tblsales')->row()->clientid;
    ...
    public function mark_as($data)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            if ($data['status_id'] == 4) {
                ...
                if ($old_status == 4) {
                    $this->db->update('tblsales', array('date_finished'=>null));
                }
            }
        ...
        if (sizeof($project_members_in) > 0) {
            ...
            if (isset($project_members)) {
                ...
                foreach ($project_members as $staff_id) {
                    ...
                    $_exists = $this->db->get('tblsalemembers')->row();
                    if (!$_exists) {
                        if (empty($staff_id)) {
                            continue;
                        }
                        $this->db->insert('tblsalemembers', array(
                            'project_id' => $id,
                            'staff_id' => $staff_id,
                        ));
                        if ($this->db->affected_rows() > 0) {
    ...
    public function is_member($project_id, $staff_id = '')
    {
        ...
        $member = total_rows('tblsalemembers', array(
            'staff_id' => $staff_id,
    ...
    public function get_sales_for_ticket($client_id)
    {
    ...
    public function get_project_settings($project_id)
    {
        ...
        return $this->db->get('tblsalesettings')->result_array();
    }

    public function get_project_members($id)
    {
        $this->db->select('email,project_id,staff_id');
        $this->db->join('tblstaff', 'tblstaff.staffid=tblsalemembers.staff_id');
        $this->db->where('project_id', $id);

        return $this->db->get('tblsalemembers')->result_array();
    }
    ...
    public function remove_team_member($project_id, $staff_id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $this->db->where('taskid IN (SELECT id FROM tblstafftasks WHERE rel_type="saleopp" AND rel_id="'.$project_id.'")');
    ...
    public function get_discussion($id, $project_id = '')
    {
        ...
        if (is_client_logged_in()) {
            ...
            $this->db->where('project_id IN (SELECT id FROM tblsales WHERE clientid=' . get_client_user_id() . ')');
        }
        $discussion = $this->db->get('tblsalediscussions')->row();
        if ($discussion) {
            return $discussion;
        }
    ...
    {
        $this->db->where('id', $id);
        $comment = $this->db->get('tblsalediscussioncomments')->row();
        ...
        if (!is_null($comment->file_name)) {
            $comment->file_url = site_url('uploads/saleoppdiscussions/' . $comment->discussion_id . '/' . $comment->file_name);
        }
    ...
    public function get_discussion_comments($id, $type)
    {
        ...
        $comments = $this->db->get('tblsalediscussioncomments')->result_array();
        $i        = 0;
        foreach ($comments as $comment) {
            ...
            if (!is_null($comment['file_name'])) {
                $comments[$i]['file_url'] = site_url('uploads/saleoppdiscussions/' . $id . '/' . $comment['file_name']);
            }
    ...
    public function get_discussions($project_id)
    {
        ...
        $discussions = $this->db->get('tblsalediscussions')->result_array();
        $i           = 0;
        foreach ($discussions as $discussion) {
            $discussions[$i]['total_comments'] = total_rows('tblsalediscussioncomments', array(
    ...
    public function add_discussion_comment($data, $discussion_id, $type)
    {
        ...
        $this->db->insert('tblsalediscussioncomments', $_data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
            if ($type == 'regular') {
                $discussion = $this->get_discussion($discussion_id);
                $not_link   = 'sales/view/' . $discussion->project_id . '?group=project_discussions&discussion_id=' . $discussion_id;
            } else {
                $discussion                   = $this->get_file($discussion_id);
                $not_link                     = 'sales/view/' . $discussion->project_id . '?group=project_files&file_id=' . $discussion_id;
            ...
            $this->log_activity($discussion->project_id, 'saleopp_activity_commented_on_discussion', $discussion->subject, $discussion->show_to_customer);
    ...
    public function update_discussion_comment($data)
    {
        ...
        $this->db->where('id', $data['id']);
        $this->db->update('tblsalediscussioncomments', array(
    ...
    public function delete_discussion_comment($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->delete('tblsalediscussioncomments');
        if ($this->db->affected_rows() > 0) {
    ...
    public function add_discussion($data)
    {
        ...
        $data['description'] = nl2br($data['description']);
        $this->db->insert('tblsalediscussions', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
            $members           = $this->get_project_members($data['project_id']);
            $notification_data = array(
                'description' => 'not_created_new_project_discussion',
                'link' => 'sales/view/' . $data['project_id'] . '?group=project_discussions&discussion_id=' . $insert_id,
            );
            ...
            $this->log_activity($data['project_id'], 'saleopp_activity_created_discussion', $data['subject'], $data['show_to_customer']);
    ...
    public function edit_discussion($data, $id)
    {
        ...
        $data['description'] = nl2br($data['description']);
        $this->db->update('tblsalediscussions', $data);
        if ($this->db->affected_rows() > 0) {
            $this->log_activity($data['project_id'], 'saleopp_activity_updated_discussion', $data['subject'], $data['show_to_customer']);
    ...
    public function delete_discussion($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->delete('tblsalediscussions');
        if ($this->db->affected_rows() > 0) {
            $this->log_activity($discussion->project_id, 'saleopp_activity_deleted_discussion', $discussion->subject, $discussion->show_to_customer);
    ...
    public function copy($project_id, $data)
    {
        ...
        $_new_data = array();
        $fields    = $this->db->list_fields('tblsales');
        ...
        $this->db->insert('tblsales', $_new_data);
    ...
    public function get_staff_notes($project_id)
    {
        $this->db->where('project_id', $project_id);
        $this->db->where('staff_id', get_staff_user_id());
        $notes = $this->db->get('tblsalenotes')->row();
        if ($notes) {
    ...
    public function save_note($data, $project_id)
    {
        ...
        if ($notes) {
            ...
            $this->db->update('tblsalenotes', array(
                'content' => $data['content'],
            ));
            if ($this->db->affected_rows() > 0) {
                return true;
            ...
            $this->db->insert('tblsalenotes', array(
                'staff_id' => get_staff_user_id(),
                'content' => $data['content'],
                'project_id' => $project_id,
            ));
    ...
    {
        $project_name = get_sales_opp_name_by_id($project_id);
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $this->db->where('fieldto', 'sales');
            $this->db->delete('tblcustomfieldsvalues');

            $this->db->where('rel_id', $project_id);
            //$this->db->where('rel_type', 'project');
            $this->db->where('rel_type', 'salesopp');
            ...
            $this->db->where('saleopp_id', $project_id);
            ...
            logActivity('Sales Opportunity Deleted [ID: ' . $project_id . ', Name: ' . $project_name . ']');
    ...
    {
        if (!is_client_logged_in()) {
            $has_permission = has_permission('sales', '', 'view');
            if (!$has_permission) {
                $this->db->where('project_id IN (SELECT project_id FROM tblsalemembers WHERE staff_id=' . get_staff_user_id() . ')');
            }
        }
        if (is_client_logged_in()) {
            $this->db->where('visible_to_customer', 1);
        ...
        $activities = $this->db->get('tblsaleactivity')->result_array();
        $i          = 0;
        foreach ($activities as $activity) {
            ...
            if (strpos($_additional_data, 'project_status_') !== false) {
                $_additional_data = get_sales_opp_status_by_id(strafter($_additional_data, 'project_status_'));
            }
            $activities[$i]['description']     = _l($activities[$i]['description_key']);
            $activities[$i]['additional_data'] = $_additional_data;
            $activities[$i]['project_name'] = get_sales_opp_name_by_id($activity['project_id']);
    ...
    public function log_activity($project_id, $description_key, $additional_data = '', $visible_to_customer = 1)
    {
        ...
        $this->db->insert('tblsaleactivity', $data);
    }
    ...
    public function new_project_file_notification($file_id, $project_id)
    {
        ...
        $this->log_activity($project_id, 'saleopp_activity_uploaded_file', $additional_data, $file->visible_to_customer);

        $members = $this->get_project_members($project_id);
        $notification_data = array(
           'description'=>'not_project_file_uploaded',
           'link'=>'sales/view/'.$project_id.'?group=project_files&file_id='.$file_id,
           );
    ...
    public function total_logged_time_by_billing_type($id, $conditions = array())
    {
        ...
        if ($project_data->billing_type == 2) {
            ...
            $data                = $this->sales_model->calculate_total_by_project_hourly_rate($seconds, $project_data->project_rate_per_hour);
    ...
    public function getProductsSO()
    {
        $this->db->select('id,description, long_description');
        $query = $this->db->get('tblproductsso');
        return $query->result_array();
    }
	 public function delete_product()
    {
        ...
        $this->db->delete('tblproductsso');
        if ($this->db->affected_rows() > 0)
        {
            $this->db->where('product_id',$del_id);
            $this->db->delete('tblproductstasksso');
            logActivity('Sales Product Deleted [ID: ' . $del_id . ']');
