                ...
                if($data['flagToShowVerseatsTab'] == true){             
                    ...
                    if($reports_data){
                        ...
                        // echo $related_merchant_versiPos; exit('lpp'); 
                            $data['versiPos']           = $this->get_versiPos_data($related_merchant_versiPos);
                            // exit('kp');
...
public function get_versiPos_data($val = ''){
        ...
        if($_POST){
            parse_str($_POST['formdata'], $formdata);
            $reports_versipos = $this->clients_model->get_versiPOS_api_data($formdata['restaurant_name_versiPos']);
            $data['versiPos'] = $reports_versipos;  
            //print_r($formdata); print_r($data); exit('kpo'); 
            ...
            return  $reports_data_versipos = $this->clients_model->get_versiPOS_api_data($related_merchant_array[0]);
        }
                ...
                if($dept_data['host'] && $dept_data['imap_username'] && $dept_data['password'] ){
                    ...
                    if ($imap->isConnected() === true) {
						...
						foreach($unreads as $unread){
                            $email = $imap->getMessageHeader($unread['uid']);
							$email_address ='';
                            $email_address .= $email->from[0]->mailbox . '@';
							$email_address .= $email->from[0]->host;
							if($email_address != ''){
								$customers = $this->tickets_model->get_companies_for_imap(trim($email_address));
							}
                            if($customers){
                                foreach ($customers as $cust) {
                                    ...
                                    if($res){
                                        $ins = array(
										'ticket_id' => $res,
										'email_subject'=>$unread['subject'],
										'created_at'=> date('Y-m-d H:i:s')
										);
										$insert = $this->db->insert('tbl_ticket_reply_email', $ins);
										//echo "i m here. "; echo  $insert; 
										$val++;
                                    }
            ...
            foreach ($departments_data as $dept_data) {
                if($dept_data['host'] && $dept_data['imap_username'] && $dept_data['password'] ){
                    ...
                    if ($imap->isConnected() === true) {
						...
						foreach($unreads as $unread){
							$body = trim(strip_tags($unread['body'])); 
							//echo "<pre>"; print_r($body); exit('jj');   
							//$body = preg_replace("/<div>(.*?)<\/div>/", "$1", $unread['body']);
							//print_r($unread['subject']);  
							$str = substr(trim($unread['subject']), 0, 3);
							if($str == 'Re:'){
								$subject = substr(trim($unread['subject']), 3);
								$this->db->select('*');
								$this->db->from('tbl_ticket_reply_email');
								$this->db->where('email_subject', trim($subject));
								$replys = $this->db->get()->result_array();
								foreach($replys as $rep){
									
									$reply_add = array();
									if($rep['email_reply'] != ''){
										$reply_add = explode('&&', $rep['email_reply']);
									}
									//echo strcmp($reply_add[0],$body);
									//exit('kojjiij'); 
									//echo "<pre>"; print_r($reply_add[0]); echo "<br>"; 
									//echo "<pre>"; print_r(trim($body)); exit('koka');  
									if(!in_array($body, $reply_add) || $rep['email_reply'] == ''){
										//echo "<pre>"; print_r($reply_add); exit('ko');  
										$tickt = $this->tickets_model->get($rep['ticket_id']);
										$data_ins = array(
										 	'status' => $tickt->status,
											'knowledgeBase' => 0,
											'ticketid' => $tickt->ticketid,
											'message' => trim($body),
											'email_reply' => '',
											'cc' => '',
											'userid' => $tickt->userid,
										);
										$replyaddd = $this->tickets_model->add_reply($data_ins, $tickt->ticketid);
										if($replyaddd){
											if($rep['email_reply'] ==''){
												$this->db->where('ticket_id', $tickt->ticketid);
												$this->db->update('tbl_ticket_reply_email', array(
													'email_reply' => trim($body)
												));
											}
											else{		 
												array_push($reply_add, trim($body));
												$implode_rep = implode('&&', $reply_add);
												$this->db->where('ticket_id', $tickt->ticketid);
												$this->db->update('tbl_ticket_reply_email', array(
													'email_reply' => $implode_rep
												));
											}
										}
									}
