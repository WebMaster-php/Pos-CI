...

{
    ...
    public function tickets($status = '')
    {
        ...
        $where = array(
            'tbltickets.contactid' => get_contact_user_id(),
            'tbltickets.portfolio_id'=> 1,
        );
    ...
    public function proposals()
    {
        ...
        echo "<pre>"; print_r($primary_contacts); exit;
        if($primary_contacts){
            $results = array();
...
//        echo "<pre>"; print_r($data['ticket']); exit; 
	...
	public function gettickets()
	{
				 ...
				 foreach($tick as $ticket){
					...
					$html .= ' <tr class="'.$text.'">
							<td data-order="'.$ticket["ticketid"].'">
									<span  style="color: #333333 !important; cursor:pointer; "><b> # '.$ticket["ticketid"].'</b></span>		
							</td>
							<td><span style="color: #333333 !important; cursor:pointer; " ><b>'.$ticket["subject"].'</b></span></td>
							 <td>
					...
					// $html .= 	'</td>
								// <td data-order="'.$ticket["lastreply"].'">
									// <div class="btn-group dropdown">
										...
										// <div class="dropdown-menu dropdown-menu-right">
											// <a class="dropdown-item" href="'.site_url("clients/ticket/".$ticket["ticketid"]).'">
		...
		else
		{
			...
			//echo "ch"; echo "<pre>";print_r($tickets); exit;
					if($tickets){
...
$data['company_for_versieats']        = $this->inventory_model->get_company($user_id);         
       ...
       // echo ""; print_r($data['company_for_versieats']); exit('kl');  
        $data['flagToShowVerseatsTab'] = ''; 
        // echo "<pre>"; print_r($data['company_for_versieats']); exit;  
        $related_merchant = ''; 
        foreach($data['company_for_versieats'] as $com){
            if($com['versieats_username'] != '' && $com['versieats_password'] !='' && $com['versieats_merchant_id'] != ''){
            ...
            if($_POST['run']){
                // echo "<pre>"; print_r($_POST); exit('in'); 
                
                $formdata = array();
                parse_str($_POST['formdata'], $formdata);
                $reports_data = $this->clients_model->reports($formdata);
                
                //$reports_data = $this->clients_model->reports($formdata['restaurant_name'], $_POST['day']);
                // echo "<pre>"; print_r($reports_data); exit('ds');      
                $merchant = $this->clients_model->merchant($formdata['restaurant_name']);
                 if($reports_data){
                        ...
                        $data['reportingon'] = date('Y-m-d H:i:s'); 
                        $data['merchant'] = $merchant; 
                        $data['report_id'] = $_POST['day']; 
                        
                    }
                // echo "<pre>"; print_r($data); exit('ds');
