...
// print_r($this->session->userdata('staff_user_id')); exit('this'); 
        if ($this->input->is_ajax_request()) {
           // echo "<pre>"; print_r($this->input->post()); exit('hellokooo');
           // echo "<pre>"; print_r($_SESSION); exit('ioo'); 
           $this->session->set_userdata('ticket_status_3', ''); 
           // $this->input->post('ticket_follow', '');   
           // $_POST['ticket_follow'] = '';
           // echo "<pre>"; print_r($this->input->post()); exit('hellokooo'); 
      ...
      $this->app->get_table_data('tickets', $tableParams);
        ...
        $data['staff']              = $this->staff_model->get_ticket_staff();

        $default_filters = get_default_filters($this->session->userdata('staff_user_id'));

        $data['default_tickets_list_statuses'] = do_action('default_tickets_list_statuses', $default_filters);
   ...
   public function add($userid = false)
    { 
        ...
        //$val = $this->company_contacts(74);
        
    if ($this->input->post()) {
      $data            = $this->input->post();
      $data['contactid']=implode(',', $this->input->post('contactid'));

       $data['userid'] = explode(',', $this->input->post('contactid'), 2);
      // if(isset($array[0]))
      // {
      //   $data['contactid'] = $array[0];
      // }else{
      //   $data['contactid'] = $this->input->post('contactid');
      // }
      $data['message'] = str_replace("class","",strip_tags(html_entity_decode($this->input->post('message',false)) ,'<p><a><span><div><strong>'));
      //zee code start
      ...
      if ($id) {
         $action = 'Ticket Name: "'.$data['subject'].'" Created ';
         $tabname = "tickets";
         $heading = "Company Tickets";
        $this->tickets_history($this->input->post('userid'), $action, $tabname,  $heading);
        ...
        $data['services']           = $this->tickets_model->get_service();
        $data['staff']              = $this->staff_model->get_ticket_staff();
        $data['staff_multi']              = $this->staff_model->get_ticket_staff_multi_portfolio();
        ...
        $data['client'] = $this->tickets_model->get_contacts();
        $data['statuses']             = $this->tickets_model->get_ticket_status();
    ...
    //bilal code start
    if ($this->input->get('saleopp_id') && $this->input->get('saleopp_id') > 0) {
            // request from project area to create new ticket
            $data['project_id'] = $this->input->get('saleopp_id');
            $data['saleopp_id'] = $this->input->get('saleopp_id');
            $data['userid'] = get_client_id_by_sales_opp_id($data['project_id']);
            if (total_rows('tblcontacts', array('active'=>1, 'userid'=>$data['userid'])) == 1) {
                $contact = $this->clients_model->get_contacts($data['userid']);
                //echo  $contact;
                if (isset($contact[0])) {
    ...
    elseif ($this->input->get('contact_id') && $this->input->get('contact_id') > 0 && $this->input->get('userid')) {
        ...
        // print_r($contact);
        // exit('koko');
        // echo "<pre>"; echo $userid; echo $contact_id;  
        // print_r($data); 
        // exit('this');
    //bilal code start
        $data['customers'] = $this->clients_model->get_all_clients_for_mail();
        $data['sources'] = $this->tickets_model->get_sources();
        // echo "<pre>"; print_r($sa);
        // echo "<pre>"; print_r($data); exit('here'); 
        $data['compnay_data']=$this->clients_model->get_1();
        // echo "<pre>"; print_r($data); exit('kkokok');  
    if($this->input->get('saleopp_id')){
      $this->load->view('admin/tickets/add_so', $data);
    }else{
    ...
    public function delete($ticketid, $company_id="")
    {
      ...
      $action = "Tickets Deleted";
      $tabname = "tickets";
      $heading = "Company Tickets";
      $this->tickets_history($company_id, $action, $tabname,  $heading);
    ...
    public function ticket($id, $company_id = '')
    {
    ...
    if (!$id) {
      ...
      // abrar
        $data['email_reply']= implode(',', $this->input->post('email_reply'));
        $mail=implode(',', $this->input->post('email_contacts_ids'));

        // $email = $mail ;
        // $html_email="email reply";
      ...
      if ($data['knowledgeBase']) {
            ...
            $data['userid'] = $this->tickets_model->get_userid_reply($id);

            $data['message'] = $this->input->post('message', false);
      //echo "<pre>"; print_r($data); exit('in cont ');   
      $replyid         = $this->tickets_model->add_reply($data, $id, get_staff_user_id()); 
      // echo $replyid;  echo "istarha"; echo "<pre>"; print_r($data);  exit('innnnmojomom');
      if($knowledgeBase == 1 && $replyid != ''){
         ...
         redirect(admin_url('knowledge_base/article/' . $success));
      } 
            if ($replyid) {
        $action = 'Tickets Name: "'.$data['subject'].'" Created ';
          $tabname = "tickets";
        $heading ="Company Tickets";
        $this->ticket_his($company_id , $action, $tabname, $heading);
    ...
    //sajid
        $res_id       = get_ticket_company_from_ticket_id($id);
        $data['client_contacts']       =  $this->tickets_model->get_contacts($res_id);
        $data['s_inventory'] = $this->tickets_model->get_inventory($res_id);
    //for inventory start
        $whereb = [];
        if(!null == $this->session->userdata('portfolio_id')){
            ...
            if($check == '')
            {
                $whereb= array('portfolio_id'=>$this->session->userdata('portfolio_id'));
            }
        }
        
        $data['values'] = $this->inventory_model->get('tblcustom', $objArr=TRUE, $whereb);       
        $data['cust'] = $this->inventory_model->get('tblclients','', $whereb);
        $data['accounts'] = $this->inventory_model->get('tblclients', $objArr=TRUE,$whereb);
        $whereb['group_type'] = 'status';
        // $data['statuses'] = $this->inventory_model->get('tblcustom','', $whereb );
        //for inventory end
        // get_reminder_company();  
        // $data['ticket']->userid
        // abrar
		$data['sources'] = $this->tickets_model->get_sources();
		$data['compnay_data']=$this->clients_model->get_1();
        $data['company_contacts'] = $this->tickets_model->get_contact_for_comapny_ticket();
        // abrar
        $data['replies'] = $this->inventory_model->get('tblticketreplies',false,array('ticketid'=>$id,'pin_to_top'=>'1'),'*');
        $data['reminders']= $this->misc_model->get_company_reminders($data['ticket']->userid);
// echo $data['ticket']->userid; echo "<pre>"; print_r($data); exit('kokoko');  
        // $ticket->userid
        $data['followers_data'] = $this->tickets_model->get_followers_data($id);
        $data['client'] = $this->tickets_model->get_contacts();
    ...
    public function change_status_ajax($id, $status, $company_id)
    {  
    $this->ticket_upadate_history($id, $status, $company_id);
    if ($this->input->is_ajax_request()) {
    ...
    public function predefined_replies()
    {
      ...
      if($this->session->userdata('portfolio_id')){
        array_push($where , 'WHERE portfolio_id = ' . $this->session->userdata('portfolio_id'));  
      }
      
      $result       = data_tables_init($aColumns, $sIndexColumn, $sTable, $where, array(), array(
                'id',
            ));
    ...
    public function ticket_change_data()
    {
      ...
      if(isset($array[0]))
      {
        ...
        $userid = $array[1];
            ...
            echo json_encode(array(
                'contact_data' => $this->clients_model->get_contact_company(array('cs1.contact_id' => $contactid, 'cs1.company_id' => $userid)),
    ...
    public function services()
    {
      ...
      if(!null == $this->session->userdata('portfolio_id'))
      {
        array_push($where ,'WHERE tblservices.portfolio_id = ' . $this->session->userdata('portfolio_id')); 
      }
    ...
    public function spam_filters($type = '')
    {
      ...
      if($this->session->userdata('portfolio_id'))
      {
        array_push($where, 'AND portfolio_id = ' . $this->session->userdata('portfolio_id'));
      }
            $sIndexColumn = "id";
  ...
  public function tickets_history($company_id, $action, $tabname, $heading)
  {
  ...
  public function ticket_upadate_history($id, $status, $company_id)
  {
    $fields_status = $this->tickets_model->get_status($status);
    $field = $this->tickets_model->get_ticket($id);
    $fields_tickets = $this->tickets_model->get_status($field[0]['status']);
    if($fields[0]['ticketstatusid'] != $status)
     {
       
       $fieldschanged = 'Expiration Date "'.$fields_tickets[0]['name'].'" updated to "'.$fields_status[0]['name'].'"';
       if ($fieldschanged){
       $values = array(
         'company_id' => $company_id,
         'user_id'     => $this->session->userdata('staff_user_id'),
         'heading'      => 'Change of Status:',
         'tabname'    =>  "tickets",
  ...
  public function ticket_his($company_id, $action , $tabname, $heading)
  {
    $user_id = $this->session->userdata('staff_user_id');
    $history = array(
            'user_id'     =>  $user_id,
  ...
  public function company_contacts($company_Id){
        ...
        $company_data_contacts=$this->clients_model->get_compny_contacts($company_Id);
        echo(json_encode($company_data_contacts));

    }
    public function get_company_contacts_id()
    {
        $email = $this->input->post();
        $store = '';
        foreach($email['val'] as $em)
        { 
            $this->db->select('id');
   ...
   public function creat_ticekt_by_imap(){
        $id = 2;
        $val = 0 ;
        $this->load->model('departments_model');
        $departments_data=$this->departments_model->get();
        //echo "<pre>"; print_r($departments_data); exit('ereeee');  
        if($departments_data){
