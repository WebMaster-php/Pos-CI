...
class Utilities extends Admin_controller
{
    ...
    public function media_connector()
    {
        ...
        if (!is_admin()) {
            $this->db->select('media_path_slug,staffid,firstname,lastname');				 //add by zeeshan 
            $this->db->from('tblstaff');													 //add by zeeshan 
            $this->db->where('staffid', get_staff_user_id());								 //add by zeeshan 
			$this->db->where('portfolio_id', $this->session->userdata('portfolio_id'));   	 //add by zeeshan 
        ...
        if ($this->input->post()) {
            ...
            if ($type == 'invoices') {
             ...
             elseif ($type == 'estimates') {
                ...
                $this->db->where('tblinvoicepaymentrecords.portfolio_id', $this->session->userdata('portfolio_id'));
