...
class Invoices extends Admin_controller
{
    ...
    public function table_so($clientid = '')
    {
        if (!has_permission('invoices', '', 'view') && !has_permission('invoices', '', 'view_own')) {
            ajax_access_denied();
        }
        ...
        $this->app->get_table_data('invoices_so', array(
            'clientid' => $clientid,
            'data' => $data,
        ));
    }	
    ...
    public function client_change_data($customer_id, $current_invoice = '')
    {
		...
		//Muneeb Added [18-10-18]
		
		if(strpos($customer_id,'_')!==false){
            $customer_id = substr($customer_id, 0, strpos($customer_id, "_"));
        }  
    ...
    public function invoice($id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if ($id == '') {
				...
				//sajid
				$invoice_data['portfolio_id'] = $this->session->userdata('portfolio_id'); 
				//sajid 
                $id = $this->invoices_model->add($invoice_data);
                if ($id) {
					$company_id = $invoice_data['clientid'];
					$action = 'Invoice Number: "'.$invoice_data['number'].'" Created ';
					$tabname = "invoices";
					$heading = "Company Invoice Created";
					$this->invoice_history($company_id, $action , $tabname, $heading);
        ...
        $data['staff']             = $this->staff_model->get('', 1 . 'AND portfolio_id = ' . $this->session->userdata('portfolio_id'));
        $data['title']             = $title;
	...
	public function invoice_history($company_id, $action , $tabname, $heading)
	{
