...
class Clients extends Admin_controller
{
    ...
    public function __construct()
    {
		...
		check_portfolio_id(); 
        $this->load->model('client_services_model');
    ...
    public function index()
    {
        ...
        $data['services']       = $this->client_services_model->get_services();
    ...
    {
		if($this->session->userdata('redirect_id_param') && $id && $_GET['new_contact'] )
		{
			$this->session->unset_userdata('redirect_id_param');
			redirect(admin_url('clients/client/'.$id)."?tab=contacts");
		}
        ...
        //$data['values'] = $this->inventory_model->get('tblcustom', $objArr=TRUE, $where = 'portfolio_id = ' . $this->session->userdata('portfolio_id') );		
        //$data['accounts'] = $this->inventory_model->get('tblclients', $objArr=TRUE, $where = 'portfolio_id = ' . $this->session->userdata('portfolio_id'));	
		...
		$data['contacts'] = $this->inventory_model->getprofileContacts('tblcontacts', $objArr=TRUE,$where, $fields='*', $order=false, $join);
        if ($this->input->post() && !$this->input->is_ajax_request()) {
            if ($id == '') {
                ...
                $data['portfolio_id'] = $this->input->post('portiflio_clients');
                unset($data['portiflio_clients']);                
                if($data['portfolio_id']){
                      $data['portfolio_id'] = $data['portfolio_id'];
                    }
                else{
				...
				if(isset($data['versiPOS_merchant_id_not'])){
					unset($data['versiPOS_merchant_id_not']);
				   }
				if(isset($data['versiPOS_store_id_not'])){
					unset($data['versiPOS_store_id_not']);
				}
                ...
                // echo "<pre>";  print_r($data); exit; 
                 $id_data =  $id;
                 $portfolio_id_update = $data['portfolio_id'];
				...
				//echo "<pre>"; print_r($data); exit('ui'); 
                $success = $this->clients_model->update($data, $id);
                if ($success == true) {				
                    
                    update_portfolio_id($id_data,$portfolio_id_update);
        ...
        if ($id == '') {
            ...
            $data['contacts_rel_clients']       = $this->clients_model->get_all_tblcontacts_rel_clients(array('company_id'=>$id) );
            // Fetch data based on groups
        // print_r($group); exit; 
            if ($group == 'profile') {
                $data['customer_groups'] = $this->clients_model->get_customer_groups($id);
                $data['customer_services'] = $this->client_services_model->get_services_selected($id);
                
                // print_r($data['customer_services']); exit; 
			...
			elseif ($group == 'statement') {
                ...
                $contact = $this->clients_model->get_contact_rel(get_primary_contact_user_id($id));
        ...
        $data['services']       = $this->client_services_model->get_services();
        $data['client_portfolio_data'] = $this->clients_model->client_portfolio_data();
    ...
    {	
        ...
        if ($this->input->post()) {
            ...
            if ($contact_id == '') {
				...
				echo json_encode(array(
                    ...
                    'message' => $message,
                    'has_primary_contact'=>(total_rowss('tblcontacts', array('userid'=>$customer_id, 'is_primary'=>1)) > 0 ? true : false),
                    'is_individual'=>is_empty_customer_company($customer_id) && total_rowss('tblcontacts',array('userid'=>$customer_id)) == 1,
                ));
                die;
            } else {
                if (!has_permission('customers', '', 'edit')) {
                ...
                echo json_encode(array(
                    ...
                    'has_primary_contact'=>(total_rowss('tblcontacts', array('userid'=>$customer_id)) > 0 ? true : false),
                ));
                die;
            }
        }
        if ($contact_id == '') {
            ...
            $data['contact'] = $this->clients_model->get_contact_rel($where);
		...
		$data['title']                = $title;
		//$join = 1;
		$where = ' tblcontacts.portfolio_id = ' . $this->session->userdata('portfolio_id'); 
		$data['contactsdata'] = $this->inventory_model->get('tblcontacts', $objArr=TRUE, $where );
    ...
    public function delete_contact($customer_id, $id)
    {
        ...
        $check = verify_client($customer_id);
        // print_r($check); exit;  
        if(!empty($check)){
                            ...
                            if (count($insert) > 0) {
                                ...
                                if (!$this->input->post('simulate')) {
                                    ...
                                    if (isset($insert['company']) && $insert['company'] != '' && $insert['company'] != '/') {
                                        if (total_rows('tblclients', array('company'=>$insert['company'],'portfolio_id'=>$this->session->userdata("portfolio_id"))) === 1) {
											...
											// adding portfolio_id to data
											$tmpInsert['portfolio_id'] = $this->session->userdata("portfolio_id");	
    ...
    public function services()
    {
        ...
        if ($this->input->is_ajax_request()) {
            $this->app->get_table_data('customers_services');
        }
        $data['title'] = _l('customer_services');
        
        $this->load->view('admin/clients/services_manage', $data);
    }
    public function service()
    {
        if (!is_admin() && get_option('staff_members_create_inline_customer_groups') == '0') {
            access_denied('Customer services');
        }
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($data['id'] == '') {
                ...
                $id = $this->client_services_model->add($data);
                $message = $id ? _l('added_successfully', _l('customer_services')) : '';
                ...
                $success = $this->client_services_model->edit($data);
                $message = '';
                if ($success == true) {
                    $message = _l('updated_successfully', _l('customer_services'));
                }
                echo json_encode(array(
                    'success' => $success,
                    'message' => $message,
    ...
    public function delete_service($id)
    {
        ...
        $response = $this->client_services_model->delete($id);
        if ($response == true) {
            set_alert('success', _l('deleted', _l('customer_services')));
        } else {
            set_alert('warning', _l('problem_deleting', _l('customer_group_lowercase')));
        }
        redirect(admin_url('clients/services'));
	...
	public function delete_contacts_rel_clients($customer_id, $id, $ticket_page_id = '')
	{
		// exit(''); 
        $company_id = $customer_id;
		$query = $this->clients_model->delete_contacts_rel_clients($customer_id, $id);
		
		// redirect(admin_url('clients/client/' . $customer_id . '?tab=contacts'));
        if($ticket_page_id){

                redirect(admin_url('tickets/ticket/' . $ticket_page_id ));   
            } 
         else{
            redirect(admin_url('clients/client/' . $customer_id . '?tab=contacts'));   
         }   
	...
	//	echo "ech"; exit ; //echo $company_id; echo $contact_id ; exit; 
		...
		$query = $this->clients_model->get_existing_check($company_id , $contact_id);
		if($query)
		{			
			 echo 1; 
		}
	...
	public function getcompanyforcontact()
	{
		$this->db->select("*");
		$this->db->from('tblclients');
		$qry = $this->db->get();
		...
		if($qry->num_rows() > 0)
		{
			...
			$html .='<div class="company_opt_appended__">';
			$html .='<select class="form-control" name ="idd">';
			$html .='<option value="">Select company</option>';
			foreach($qry->result_array() as $res)
			{
				$html .='<option  value="'.$res['userid'].'">'.$res['company'].'</option>';
			}
			$html .='</select>';
			$html .='</div>';
		}
        ...
        /* Add or update lead */
    public function lead($id = '',$client_id='')
    {
        ...
        if ($this->input->post()) {
            //echo $id;
            // echo "<pre>"; print_r($this->input->post()); exit('here'); 
         ...
         if ($id == '') {
            ...
            $data['portfolio_id'] = $this->session->userdata('portfolio_id'); 
            $id      = $this->clients_model->add_customer_leads($data);
            $message = $id ? _l('added_successfully', "Sale opportunity") : '';
           ...
           // $data['client_id']=$client_id;
            $success          = $this->leads_model->updateCustomerLeads ($data, $id);
            if ($success) {
                ...
                $message = _l('updated_successfully',"Sale opportunity");
            }
               //salik code start
            if($leadData){
            ...
            echo json_encode(array(
                ...
                'leadView'=>$this->_get_lead_data($id, $client_id),
            ));
        }
        die;
    }

    echo json_encode(array('leadView'=>$this->_get_lead_data($id, $client_id)));

}
private function _get_lead_data($id = '', $client_id ='')
{
    $reminder_data = '';
    ...
    if (is_numeric($id)) {
        ...
        $lead = $this->clients_model->get_customers_lead($id, $leadWhere);
    ...
    $data['client_id']  = $lead->client_id ? $lead->client_id : $client_id;

    $data = do_action('lead_view_data', $data);
    // echo "<pre>"; print_r($data) ; exit('kokok'); 
    return array(
        'data' => $this->load->view('admin/clients/customer_leads', $data, true),
        'reminder_data' => $reminder_data,
    );
}
	
public function sales_oppertunities($client_id)
    {  
        $this->app->get_table_data('customerleads', array(
            'userid' => $client_id,
        ));
    }
    public function customer_lead_delete($id)
{
    if (!$id) {
    ...
    $response = $this->clients_model->delete_customer_lead($id);
    if (is_array($response) && isset($response['referenced'])) {
        set_alert('warning', _l('is_referenced', _l('lead_lowercase')));
    } elseif ($response === true) {
        set_alert('success', _l('deleted', "Sale opportunity"));
    } else {
        set_alert('warning', _l('problem_deleting', "Sale opportunity"));
    }
	...
	public function get_custom_client_data()
{
    $id = $this->input->post('val');
    $data = $this->clients_model->get($id);
    
    //$res = $this->load->view('admin/clients/lead_profile', $data, true);
    //echo "<pre>"; print_r($data); exit('fenddddd');
    echo json_encode($data);
}
	
	public function costumer_contact($customer_id, $contact_id = '')
    {	
		...
		$this->load->view('admin/clients/clientContactsModal', $data);
