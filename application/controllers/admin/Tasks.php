...
class Tasks extends Admin_controller
{
    ...
    {  
        ...
        if ($this->input->post()) {
            ...
            if ($id == '') {
                ...
                $message = '';
                if ($id) {
					$action = 'Task Name: "'.$data['name'].'" Created ';
					$tabname = "tasks";
					$heading = "Company Tasks";
					$this->task_history($this->input->post('rel_id'), $action, $tabname, $heading);
				...
				$this->task_history_update($data, $id, $this->input->post('rel_id'));
                $success = $this->tasks_model->update($data, $id);
                $message = '';
                if ($success) {
					// $action = 'Task Name: "'.$data['name'].'" Updated ';
					// $tabname = "tasks";
					// $heading = "Company Tasks";
					// $this->task_history($this->input->post('rel_id'), $action, $tabname, $heading);
        ...
        if ($id == '') {
			...
			if ($data['task']->rel_type == 'saleopp') {
                $data['milestones'] = $this->sales_model->get_milestones($data['task']->rel_id);
            }
		...
		if ($this->input->get('rel_type') == 'saleopp' && $this->input->get('rel_id')) {
            $project = $this->sales_model->get($this->input->get('rel_id'));
            if ($project->deadline) {
                $data['project_end_date_attrs'] = array(
                    'data-date-end-date' => $project->deadline,
                );
	...
	public function task_history($company_id, $action, $tabname, $heading)
	{
	...
	public function task_history_update($data, $id, $company_id)
	{	
		$fields = $this->tasks_model->get_related_data($id);
		
		if($fields[0]['name'] != $data['name'])
		 {

			 $fieldschanged = 'Subject "'.$fields[0]['name'].'" updated to "'.$data['name'].'"';
			 if ($fieldschanged)
			 {	 
				 $values = array(
					 'company_id' => $company_id,
					 'user_id'	   => $this->session->userdata('staff_user_id'),
					 'tabname' 		=>  "tasks",
					 'heading'			=> 'Change of Subject in Tasks:',
		...
		if($fields[0]['description'] != $data['description'])
		 {
			 ...
			 {	 
				 $values = array(
					 ...
					 'user_id'	   => $this->session->userdata('staff_user_id'),
					 'tabname' 		=>  "tasks",
					 'heading'			=> 'Change of Description in Tasks:',
					 'action'       => $fieldschanged,
					 );
				$this->clients_model->create_history($values);
			 }
		 }
		if($fields[0]['hourly_rate'] != $data['hourly_rate'])
		 {
			 $fieldschanged = 'Hourly Rate "'.$fields[0]['hourly_rate'].'" updated to "'.$data['hourly_rate'].'"';
			 if ($fieldschanged)
			 {	 
				 $values = array(
					 'company_id' => $company_id,
					 'user_id'	   => $this->session->userdata('staff_user_id'),
					 'tabname' 		=>  "tasks",
					 'heading'			=> 'Change of Hourly Rate in Tasks:',
					 'action'       => $fieldschanged,
					 );
				 $this->clients_model->create_history($values);
		...
		if($fields[0]['duedate'] != date('Y-m-d',strtotime($data['duedate'])))
		 {
			 $fieldschanged = 'Due Date "'.date('m-d-Y',strtotime($fields[0]['duedate'])).'" updated to "'.$data['duedate'].'"';
			 if ($fieldschanged)
			 {	 
				 $values = array(
					 'company_id' => $company_id,
					 'user_id'	   => $this->session->userdata('staff_user_id'),
					 'tabname' 		=>  "tasks",
					 'heading'			=> 'Change of Due Date in Tasks:',
					 'action'       => $fieldschanged,
					 );
				  $this->clients_model->create_history($values);
		...
		if($fields[0]['startdate'] != date('Y-m-d',strtotime($data['startdate'])))
		 {
			 $fieldschanged = 'Start Date "'.date('m-d-Y',strtotime($fields[0]['startdate'])).'" updated to "'.$data['startdate'].'"';
			 if ($fieldschanged)
			 {	 
				 $values = array(
					 'company_id' => $company_id,
					 'user_id'	   => $this->session->userdata('staff_user_id'),
					 'tabname' 		=>  "tasks",
					 'heading'			=> 'Change of Start Date in Tasks:',
					 'action'       => $fieldschanged,
					 );
				  $this->clients_model->create_history($values);
		...
		// if($fields[0]['milestone'] != $data['milestone'])
		 // {
			 // $fieldschanged = 'Milestone "'.$fields[0]['milestone'].'" updated to "'.$data['milestone'].'"';
			 // if ($fieldschanged)
			 // {	 
				 // $values = array(
					 // 'company_id' => $company_id,
		 ...
		 // if($fields[0]['priority'] != $data['priority'])
		 // {
			 // $fieldschanged = 'priority "'.$fields[0]['priority'].'" updated to "'.$data['priority'].'"';
			 // if ($fieldschanged)
			 // {	 
				 // $values = array(
					 // 'company_id' => $company_id,
		 ...
		 // if($fields[0]['repeat_every'] != $data['repeat_every'])
		 // {
			 // $fieldschanged = 'Repeat Every "'.$fields[0]['repeat_every'].'" updated to "'.$data['repeat_every'].'"';
			 // if ($fieldschanged)
			 // {	 
				 // $values = array(
					 // 'company_id' => $company_id,
					 // 'user_id'	   => $this->session->userdata('staff_user_id'),
					 // 'tabname' 		=>  "tasks",
					 // 'heading'			=> 'Change of Repeat Every:',
					 // 'action'       => $fieldschanged,
					 // );
				  // $this->clients_model->create_history($values);
			 ...
			 // }
		 // }		 
		 // if($fields[0]['repeat_every_custom'] != $data['repeat_every_custom'])
		 // {
			 // $fieldschanged = 'Related To"'.$fields[0]['repeat_every_custom'].'" updated to "'.$data['repeat_every_custom'].'"';
			 // if ($fieldschanged)
			 // {	 
				 // $values = array(
					 // 'company_id' => $company_id,
			 ...
			 // }
		 // }
		 // if($fields[0]['repeat_type_custom'] != $data['repeat_type_custom'])
		 // {
			 // $fieldschanged = 'Customer"'.$fields[0]['repeat_type_custom'].'" updated to "'.$data['repeat_type_custom'].'"';
			 // if ($fieldschanged)
			 // {	 
				 // $values = array(
					 // 'company_id' => $company_id,
		 ...
		 // {
			 ...
			 // }
		 // }
		 // if($fields[0]['recurring_ends_on'] != $data['recurring_ends_on'])
		 // {
			 // $fieldschanged = 'Customer"'.$fields[0]['recurring_ends_on'].'" updated to "'.$data['recurring_ends_on'].'"';
			 // if ($fieldschanged)
			 // {	 
				 // $values = array(
					 // 'company_id' => $company_id,
