...
class Proposals extends Admin_controller
{
	...
	//print_r($this->input->post());exit;
            ...
            if ($this->db->affected_rows() > 0) {
                //$this->sync_data_history($this->input->post('rel_id'));
    ...
    {	
        if ($this->input->post()) {
		   ...
		   if ($id == '') {
				...
				//zee code start	
				$proposal_data['portfolio_id'] = $this->session->userdata('portfolio_id'); 
				//zee code start
				...
				{	
					...
					$action = 'Perposal: "'.$subject.'" Created ';
					$tabname ="proposals";
					$heading = "Company Perposals";
					$this->proposal_history($company_id, $action , $tabname, $heading );
	...
	public function get_all_primary_contact($rel_id)
	{
		$this->db->select('tblcontacts.firstname, tblcontacts.lastname, tblcontacts.id, tblcontacts.email, tcorel.is_primary, tcorel.contact_id');
		$this->db->from('tblcontacts');
		$this->db->join('tblcontacts_rel_clients AS tcorel','tcorel.contact_id = tblcontacts.id','INNER');
		$this->db->where('tcorel.company_id', $rel_id);
		//$this->db->where('tcorel.is_primary', 1);
		$this->db->group_by('tcorel.contact_id');
	...
	public function proposal_history($company_id , $action , $tabname , $heading)
	{
	...
	// public function sync_data_history($company_id)
	// {
