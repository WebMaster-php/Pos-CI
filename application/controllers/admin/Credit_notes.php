...
class Credit_notes extends Admin_controller
{
    ...
    public function credit_note($id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if ($id == '') {
				...
				$credit_note_data['portfolio_id'] = $this->session->userdata('portfolio_id');
                $id = $this->credit_notes_model->add($credit_note_data);
                if ($id) {
					$action = 'Credit Note Number: "'.$credit_note_data['number'].'" Created ';
					$tabname = "credit_notes";
					$heading ="Company Credit Notes";
					$this->creadit_notes_history($this->input->post('clientid'), $action , $tabname, $heading);
	...
	public function creadit_notes_history($company_id, $action, $tabname, $heading )
	{
