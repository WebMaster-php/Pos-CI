...
class Staff extends Admin_controller
{
    ...
    public function member($id = '')
    {
        ...
        if ($this->input->post()) {
			...
			//ab stafff controller k member function ma//
                $data['ticket_permissions']=serialize($this->input->post('ticket_permissions'));
                 $data['ticket_permissions']=serialize($this->input->post('ticket_permissions'));
                 if(!isset($data['notify_inventory'])){
                    $data['notify_inventory'] = 0;
                 }
                 if(!isset($data['allow_ticket_filters'])){
                    $data['allow_ticket_filters'] = 0;
                 }
                // ab code end //
            // echo "<pre>"; print_r($data); exit('ko');  
            ...
            if ($id == '') {
                if (!has_permission('staff', '', 'create')) {
                 ...
                 // <!-- faisal code starts-->
                $data['tbl_staff_portfolio'] = $this->input->post('tbl_staff_portfolio', false);
                 // <!-- faisal code end-->
                ...
                // echo "<pre>"; print_r($data); exit('hash'); 
		...
		if($id)
			{
				$data['portfolio']		= get_portfolio_data_member($id);
				$data['permissions'] 	= $this->roles_model->get_permissions_port($id);
			}
		$data['allportfolio']		= get_all_portfolio();	
        //ab stafff controller k member function ma//
        $this->load->model('tickets_model');
        $data['statuses']= $this->tickets_model->get_ticket_status();
        // ab//
        $data['sources'] = $this->tickets_model->get_sources();
        $data['custom_inventory'] = $this->inventory_model->get('tblcustom',false,array('group_type'=>'status' , 'portfolio_id'=> $this->session->userdata('portfolio_id')),'*');
        $data['staff_inventory_notify'] = $this->inventory_model->get('tblstaff',false,array('staffid'=>$id),'*');
		$data['title']       	= $title;
    ...
    public function profile($id = '')
    {
		...
		//echo "<pre>"; print_r($total_notifications); exit; 
        $data['total_pages']       = ceil($total_notifications / $this->misc_model->get_notifications_limit());
        // echo "<pre>"; print_r($data['total_pages']); exit; 
        // ab staff controller k profile function ma//
        $this->load->model('tickets_model');
        $data['statuses']= $this->tickets_model->get_ticket_status();
        ////ab ///
        $data['sources'] = $this->tickets_model->get_sources();
    ...
    public function staff_setting(){
        if($this->input->post()){
           $update=  $this->staff_model->staff_setting($this->input->post());    
           if($update){
                set_alert('success', _l('updated_successfully', 'Profile'));
