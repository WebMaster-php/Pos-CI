...
class Expenses extends Admin_controller
{
    ...
    {	//echo $this->input->post('clientid');exit;
        if ($this->input->post()) {	
            if ($id == '') {
                ...
                if ($id) {		
						$action = 'Company Expense Name: "'.$this->input->post('expense_name').'" Created ';
						$tabname = "expenses";
						$heading = "Company Expenses";
						$this->expense_history($this->input->post('clientid'), $action, $tabname, $heading);
	...
	public function expense_history($company_id, $action, $tabname, $heading)
	{
