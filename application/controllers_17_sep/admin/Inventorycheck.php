...
class Inventory extends Admin_controller
{
            ...
            if ($this->input->post()) {
                ...
                if ($data['itemid'] == '') {
                    ...
                    echo json_encode(array(
                        ...
                        //'item' => $this->inventory_model->get($id)
                    ));
                } else {
					...
					//$success = $this->inventory_model->edit($data);
                    $message = '';
                    if ($success) {
        ...
        if (has_permission('items', '', 'view')) {
            if ($this->input->post()) {
                ...
                if ($data['itemid'] == '') {
                    ...
                    $id      = $this->invoice_items_model->add_product($data);
                    $success = false;
                    $message = '';
                    if ($id) {
                        $success = true;
                    ...
                    echo json_encode(array(
                        'success' => $success,
                        'message' => $message,
                        'item' => $this->invoice_items_model->get_product($id)
                    ));
                } else {
                    if (!has_permission('items', '', 'edit')) {
                        header('HTTP/1.0 400 Bad error');
                    ...
                    $success = $this->invoice_items_model->edit_product($data);
                    $message = '';
                    if ($success) {
                        $message = _l('updated_successfully', _l('invoice_item'));
                    }
    ...
    public function delete_product($id)
    {
        ...
        if (!$id) {
            redirect(admin_url('projects/products'));
        }

        $response = $this->invoice_items_model->delete_product($id);
        if (is_array($response) && isset($response['referenced'])) {
            set_alert('warning', _l('is_referenced', _l('invoice_item_lowercase')));
        } elseif ($response == true) {
            set_alert('success', _l('deleted', _l('invoice_item')));
    ...
    {
        if ($this->input->is_ajax_request()) {
            $item                   = $this->products_model->get_product_new($id);
            $item->productTasks     = $this->products_model->get_product_tasks($id);
			$item->long_description = nl2br($item->long_description);
            $item->custom_fields_html = render_custom_fields('items',$id,array(),array('items_pr'=>true));
            $item->custom_fields = array();
	...
	public function get_product_by_id_so($id)
    {
        if ($this->input->is_ajax_request()){
            $item                   = $this->products_model->get_product_new_so($id);
            $item->productTasks     = $this->products_model->get_product_tasks_so($id);
			$item->long_description = nl2br($item->long_description);
            $item->custom_fields_html = render_custom_fields('items',$id,array(),array('items_pr'=>true));
            $item->custom_fields = array();
	...
	public function add_new_products_so()
    {
        if (has_permission('sales', '', 'view')) {
            if ($this->input->post()) {
                ...
                if ($data['itemid'] == '') {
                    ...
                    $id      = $this->invoice_items_model->add_product_so($data);
                    $success = false;
                    $message = '';
                    if ($id) {
                        $success = true;
                    ...
                    echo json_encode(array(
                        'success' => $success,
                        'message' => $message,
                        'item' => $this->invoice_items_model->get_product_so($id)
                    ));
                } else {
                    ...
                    $success = $this->invoice_items_model->edit_product_so($data);
                    $message = '';
                    if ($success) {
                        $message = _l('updated_successfully', _l('invoice_item'));
                    }
