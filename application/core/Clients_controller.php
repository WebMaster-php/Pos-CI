...
class Clients_controller extends CRM_Controller
{
	...
	{	
		...
		$this->template['head_new'] = '';
        if ($this->use_head == true) {
            $this->template['head_new'] = $this->load->view('themes/' . active_clients_theme() . '/head_new', $this->data, true);
        }
		 $this->template['view'] = $this->load->view('themes/' . active_clients_theme() . '/views/' . $this->view, $this->data, true);
		 
        $this->template['footer_new'] = '';
        if ($this->use_footer == true) {
            $this->template['footer_new'] = $this->load->view('themes/' . active_clients_theme() . '/footer_new', $this->data, true);
        }
		$this->template['scripts_new'] = '';
        if ($this->add_scripts == true) {
            $this->template['scripts_new'] = $this->load->view('themes/' . active_clients_theme() . '/scripts_new', $this->data, true);
        }		
        $this->load->view('themes/' . active_clients_theme() . '/index_new', $this->template);
	}
    public function layout_new_1()
    { 
