...
$sTable = 'tblcontacts';
$join = array(
		'LEFT JOIN tblcontacts_rel_clients ON tblcontacts_rel_clients.contact_id = tblcontacts.id',
);
$CI =& get_instance();
...
if (!null == $CI->session->userdata('portfolio_id')) {
    array_push($where, 'AND tblcontacts.portfolio_id = ' . $CI->session->userdata('portfolio_id') );
}

//$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, array('tblcontacts.id as id', 'tblcontacts.userid', 'tblcontacts_rel_clients.is_primary','(SELECT count(*) FROM tblcontacts c WHERE c.userid=tblcontacts.userid) as total_contacts'));
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, array('tblcontacts.id as id', 'tblcontacts.userid', '(SELECT count(*) FROM tblcontacts c WHERE c.userid=tblcontacts.userid) as total_contacts'),  'GROUP BY tblcontacts.id');
...
foreach ($rResult as $aRow) {
    ...
    // if(empty($aRow['company'])){
        $companies = getCompanies($aRow['id']);
		if($aRow['id'] == 40){
		// echo "<pre>"; 
		// print_r($companies);
		// exit;
