...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == 'name') {
            ...
            $_data = _d($_data);
            if (date('Y-m-d') > $aRow['due_date'] && total_rows('tblstafftasksso', array(
                'milestone' => $aRow['id'],
                'status !=' => 5,
                'rel_id' => $project_id,
                'rel_type' => 'project'
...
if(has_permission('sales','','delete')){
    $options .= icon_btn('sales/delete_milestone/' . $project_id . '/' . $aRow['id'], 'remove', 'btn-danger _delete');
