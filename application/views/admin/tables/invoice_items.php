...
$sTable       = 'tblitems';
$CI =& get_instance();
$port = $CI->session->userdata('portfolio_id'); 
$where  	  = array(); 
$join             = array(
...
// array_push($where,'OR tblitems_groups.portfolio_id='.$CI->session->userdata('portfolio_id'));	
...
if(!null == $CI->session->userdata('portfolio_id')){
	array_push($where,'AND tblitems.portfolio_id='.$CI->session->userdata('portfolio_id'));
