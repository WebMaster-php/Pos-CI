...
if(!null == $CI->session->userdata('portfolio_id'))
	{
		//array_push($where , ' WHERE tbldepartments.portfolio_id = ' . $CI->session->userdata('portfolio_id'));	
	}
$result  = data_tables_init($aColumns, $sIndexColumn, $sTable,$where,array(),array('departmentid','email','hidefromclient','host','encryption','password','delete_after_import','imap_username'));
