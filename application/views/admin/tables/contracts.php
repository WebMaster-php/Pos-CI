...
if (!null == $CI->session->userdata('portfolio_id')) {
    array_push($where, ' AND tblcontracts.portfolio_id = ' . $CI->session->userdata('portfolio_id') );
}
...
foreach ($rResult as $aRow) {
    ...
    if (has_permission('contracts', '', 'delete')) {
        $options .= icon_btn('contracts/delete/'.$aRow['id'].'/'.$aRow['client'], 'remove', 'btn-danger _delete');
    }
    $row[] = $options;
