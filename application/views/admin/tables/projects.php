...
if(!null == $CI->session->userdata('portfolio_id')){
	array_push($where,'AND tblprojects.portfolio_id='.$CI->session->userdata('portfolio_id'));
}
...
foreach ($rResult as $aRow) {
    ...
    if ($hasPermissionEdit) {
        $options .= icon_btn('projects/project/' . $aRow['id'].'/'.$aRow['clientid'], 'pencil-square-o');
    }

    if ($hasPermissionDelete) {
        $options .= icon_btn('projects/delete/' . $aRow['id'].'/'.$aRow['clientid'], 'remove', 'btn-danger _delete');
    }
