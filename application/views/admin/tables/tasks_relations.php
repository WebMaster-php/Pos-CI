<?php
// echo "<pre>"; print_r($_POST); exit('in this');
defined('BASEPATH') or exit('No direct script access allowed');

	$CI =& get_instance();
	
	$queryClientId = "SELECT * FROM tblleads WHERE id = $rel_id";
...
// echo "<pre>"; print_r($rel_type); exit('in this');
if (!$this->ci->input->post('tasks_related_to')) {
    if(isset($rel_type) && $rel_type == 'all'){
         array_push($where, 'AND ((rel_id IN (SELECT userid FROM tblclients WHERE userid= ' . $rel_id . ') AND rel_type="customer") OR (rel_id IN (SELECT id FROM tblprojects WHERE clientid=' . $rel_id . ') AND rel_type="project") OR (rel_id IN (SELECT id FROM tblinvoices WHERE clientid=' . $rel_id . ') AND rel_type="invoice") OR (rel_id IN (SELECT id FROM tblestimates WHERE clientid=' . $rel_id . ') AND rel_type="estimate") OR (rel_id IN (SELECT id FROM tblcontracts WHERE client=' . $rel_id . ') AND rel_type="contract") OR (rel_id IN (SELECT ticketid FROM tbltickets WHERE userid=' . $rel_id . ') AND rel_type="ticket") OR (rel_id IN (SELECT id FROM tblexpenses WHERE clientid=' . $rel_id . ') AND rel_type="expense") OR (rel_id IN (SELECT id FROM tblproposals WHERE rel_type=' . $rel_id . ' AND rel_type="customer") AND rel_type="proposal"))' );
     } else  if(isset($rel_type) && $rel_type == 'lead'){
		 array_push($where, 'AND rel_id="' . $rel_id . '" OR rel_id="' . $clientId . '"'); 
	} else {
    ...
    foreach ($tasks_related_to as $rel_to) {
        ...
        }elseif ($rel_to == 'saleopp') {
            $rel_to_query .= '(rel_id IN (SELECT id FROM tblsales WHERE clientid=' . $rel_id . ')';
        }
...
// echo "<pre>"; print_r($where); exit('in this');
