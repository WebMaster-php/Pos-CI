...
$hasPermissionEdit = has_permission('announcement_perm', '', 'edit');
$hasPermissionDelete = has_permission('announcement_perm', '', 'delete');
...
if(!null == $CI->session->userdata('portfolio_id')){
	array_push($where,'AND tblannouncements.portfolio_id='.$CI->session->userdata('portfolio_id'));
} 
...
foreach ($rResult as $aRow) {
	...
	*/
    ...
    if (is_admin() || $hasPermissionEdit) 
	{
       $options .= icon_btn('announcements/announcement/' . $aRow['announcementid'], 'pencil-square-o');
    }

    if (is_admin() || $hasPermissionDelete) 
	{
        $options .= icon_btn('announcements/delete/' . $aRow['announcementid'], 'remove', 'btn-danger _delete');
    }
	$row[] = $options;
