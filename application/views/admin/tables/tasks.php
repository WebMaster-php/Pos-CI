...
if (count($custom_fields) > 4) {
	...
	if(!null == $CI->session->userdata('portfolio_id')){
		array_push($where,'AND tblstafftasks.portfolio_id='.$CI->session->userdata('portfolio_id'));
	}
...
foreach ($rResult as $aRow) {
	...
	if($aRow['hard_date']==1){
        ...
        $row[] = _d($aRow['duedate']).' &nbsp; &nbsp; <p style="color:red" class="glyphicon glyphicon-info-sign"></p>';
    }else{
