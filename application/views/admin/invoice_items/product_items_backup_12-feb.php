            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                     ...
                     <!-----start--->
							 <button type="button" id="task_submit_data_add">add data</button>
					...
					<div class="main-but">
							 <div class="form-group one_element">							 
                                ...
                                <select class="selectpicker display-block task_select" style="width:50%" name="tasks[]" data-none-selected-text="<?php echo _l('select'); ?>">
                                    <option value=""></option>
                                    <?php 
									foreach($tasks as $task){ ?>
                                    <option value="<?php echo $task['id']; ?>" data-subtext="<?php echo $task['task_name']; ?>"></option>
                                    <?php } ?>
                                </select>
...
var allTasksIds = [];
 <?php
  ...
  {
   ?>
    allTasksNames.push("<?php echo $task['task_name'];?>");
        ...
        if (typeof (id) !== 'undefined') {
            ...
            requestGetJSON('invoice_items/get_product_by_id/' + id).done(function (response) {
				...
				for( K=0; K < response.productTasks.length; K++)
				{
					...
					appendedHtml = appendedHtml + '</select><button type="button" style="height: 36px;" class="apend_task fa fa-plus"></button></div>';
