<div class="modal fade" id="sales_item_modal_so" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <?php echo form_open('admin/invoice_items/add_new_products_so', array('id' => 'invoice_item_form')); ?>
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <!-----start--->
                        <button type="button" class="btn btn-info" id="task_submit_data_add">Create a new task</button>
                        <input type="text" placeholder=" Type a task name here" name="task" id="task"><br>
                        ...
                        <div class="main-but">
                            <div class="form-group one_element">
                                ...
                                <select class="selectpicker display-block task_select" style="width:50%" name="tasks[]" id = "t"
...
<script>
    ...
    function manage_invoice_items(form) {
        ...
        $.post(url, data).done(function (response) {
            ...
            $('#sales_item_modal_so').modal('hide');
        }).fail(function (data) {
    ...
    function init_item_js() {
        ...
        $("body").on('change', 'select[name="item_select"]', function () {
            ...
            if (itemid != '' && itemid !== 'newitem') {
                ...
                // New item
                $('#sales_item_modal_so').modal('show');
            }
        });

        // Items modal show action
        $("body").on('show.bs.modal', '#sales_item_modal_so', function (event) {
            $('.affect-warning').addClass('hide');

            var $itemModal = $('#sales_item_modal_so');
            ...
            if (typeof (id) !== 'undefined') {
                ...
                requestGetJSON('invoice_items/get_product_by_id_so/' + id).done(function (response) {
                    ...
                    for (K = 0; K < response.productTasks.length; K++) {
                        ...
                        var appendedHtml = appendedHtml + '<select class="selectpicker display-block task_select" style="width:50%" name="tasks[]" data-none-selected-text="Select a Task"><option value=""></option>';
        ...
        $('#task_submit_data_add').on('click', function (e) {
            ...
            $.ajax(
                {
                    url: '<?php echo base_url(); ?>admin/sales/insert_task_project_so',
					data:{
						task:task,
						},
                    type: 'POST',
                    success: function (data) {
                       ...
                       console.log(data);
                         $('.selectpicker').append('<option value="'+myObj[0].id+'" selected="selected">'+myObj[0].name+'</option>');
                         var length = $('.dropdown-menu ul li').length;
                         $('.selectpicker').append('<option value="'+myObj[0].id+'" selected="selected">'+myObj[0].name+'</option>');
                         $('.task_select').prev().find('ul').append('<li data-original-index="' + length +'"><a tabindex="0" class="selected" data-tokens="null" role="option" aria-disabled="false" aria-selected="false"><span class="text"><small class="text-muted">'+myObj[0].name+'</small></span><span class="glyphicon glyphicon-ok check-mark"></span></a></li>'); 

                         // $('.dropdown-menu').append('<li data-original-index="' + length +'"><a tabindex="0" class="" data-tokens="null" role="option" aria-disabled="false" aria-selected="false"><span class="text"><small class="text-muted">'+myObj[0].name+'</small></span><span class="glyphicon glyphicon-ok check-mark"></span></a></li>');
                         $(".filter-option").html('<small class="text-muted">'+myObj[0].name+'</small>');
                         // $(".dropdown-toggle").attr(myObj[0].name);
                         $("input[name = task]").val('');
                       $('#task').val('');
...
/////////////////////////////////////////////////////
        $("body").on("hidden.bs.modal", '#sales_item_modal_so', function (event) {
