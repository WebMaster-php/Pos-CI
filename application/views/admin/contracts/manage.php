                ...
                <div class="panel-body _buttons">
                    ...
                    <div class="row" id="contract_summary">
                        ...
                        <div class="col-md-2 col-xs-6 border-right">
                            <h3 class="bold">
                            <?php echo total_rows('tblcontracts','(DATE(dateend) >"'.date('Y-m-d').'" AND trash=0' . (count($where_own) > 0 ? ' AND addedfrom='.get_staff_user_id() : '').') OR (DATE(dateend) IS NULL AND trash=0'.(count($where_own) > 0 ? ' AND addedfrom='.get_staff_user_id() : '').') ' . ' AND portfolio_id = ' . $this->session->userdata('portfolio_id') ); ?>
                            </h3>
                            <span class="text-info"><?php echo _l('contract_summary_active'); ?></span>
                        </div>
                        <div class="col-md-2 col-xs-6 border-right">
							<?php $where_own['portfolio_id'] = $this->session->userdata('portfolio_id'); ?>
                        ...
                        <div class="col-md-2 col-xs-6 border-right">
                            <h3 class="bold"><?php
                                echo total_rows(
                                'tblcontracts','dateend BETWEEN "'.$minus_7_days.'" AND "'.$plus_7_days.'" AND trash=0 AND dateend is NOT NULL AND dateend >"'.date('Y-m-d').'"' . (count($where_own) > 0 ? ' AND addedfrom='.get_staff_user_id() : '')  . ' AND portfolio_id = ' . $this->session->userdata('portfolio_id') ); ?></h3>
                                <span class="text-warning"><?php echo _l('contract_summary_about_to_expire'); ?></span>
                            </div>
                            <div class="col-md-2 col-xs-6 border-right">
                                <h3 class="bold"><?php
                                    echo total_rows('tblcontracts','dateadded BETWEEN "'.$minus_7_days.'" AND "'.$plus_7_days.'" AND trash=0' . (count($where_own) > 0 ? ' AND addedfrom='.get_staff_user_id() : '')   . ' AND portfolio_id = ' . $this->session->userdata('portfolio_id') ); ?></h3>
