      ...
      <?php
         ...
         foreach($menu_active->aside_menu_active as $item){
          ...
          elseif($item->id == 'customers'){ 
            if(!empty($item->permission) && !has_permission('customers','','view') ) {
              continue;
              //$item->permission = '';
            }
          }
           ...
           elseif($item->id == 'vmap'){  
            if(!empty($item->permission) && !has_permission('vmaps','','view') ) {
              continue;
              // $item->permission = '';
            }
          }
          elseif($item->id == 'hidden_items'){  
          ...
          elseif($item->id == 'projects'){ 
           // print_r(has_permission('projects','','view')); exit; 
            if(!empty($item->permission) && !has_permission('projects','','view') ) {
            // $item->permission = '';
              continue;
              //
            }
          }
          elseif($item->id == 'sales_opp'){ 
            // print_r(has_permission('sales','','view')); exit; 
            if(!empty($item->permission) && !has_permission('sales','','view') ) {
               continue;
             // $item->permission = '';
            }
          }
          // elseif($item->id == 'utilities'){ 
          //   if(!empty($item->permission) && !has_permission('utilities','','view') ) {
          //     continue;
          //     //$item->permission = '';
          //   }
          // }
          elseif($item->id == 'leads'){ 
            if(!empty($item->permission) && !has_permission('leads','','view') ) {
              continue;
              //$item->permission = '';
            }
          }
         // elseif($item->id == 'contracts'){ 
         //    if(!empty($item->permission) && !has_permission('contracts','','view') ) {
         //      continue;
         //      //$item->permission = '';
         //    }
         //  }
		...
		elseif($item->id == 'child-inventory')
		{
            if(!has_permission('inventory_items','','view') && !has_permission('inventory_items','','create')){
                continue;
            }
		} 
		else if($item->id == 'child-inventory'){
			if(!empty($item->permission) && !has_permission('inventory_items','','view')){
				continue;
			}
		}
		else if($item->id == 'child-announcements'){
			if(!empty($item->permission) && !has_permission('announcement_perm','','view')){
				continue;
			}
		}
		
		else if($item->id == 'custom-fields'){
			if(!empty($item->permission) && !has_permission('custom_fields','','view')){
				continue;
			}
		}

		else if($item->id == 'vmaps'){
			if(!empty($item->permission) && !has_permission('vmaps','','view')){
				continue;
			}
		}
		...
		elseif(!empty($item->permission)
         ...
         if(!empty($_sub_menu_check->permission)
          && ($_sub_menu_check->permission != 'payments'
			...
			&& $_sub_menu_check->permission != 'announcements'
			&& $_sub_menu_check->permission != 'vmaps'
         ...
         }elseif($_sub_menu_check->id == 'child-inventory'){
            if(!has_permission('inventory_items','','view')  && !has_permission('inventory_items','','create')){
                $total_sub_items_removed++;
            }
		 }
		
		else if($_sub_menu_check->id == 'child-announcements')
		{
		   if(!has_permission('announcement_perm','','view')){
			$total_sub_items_removed++;
		   }
		}		

		else if($_sub_menu_check->id == 'vmaps')
		{
		   if(!has_permission('vmaps','','view')){
			$total_sub_items_removed++;
		   }
		}
            ...
            <?php foreach($item->children as $submenu){
               ...
               if(
                ...
                && ($submenu->permission != 'payments'
                  && $submenu->permission != 'inventory'
				   && $submenu->permission != 'announcements'
				   && $submenu->permission != 'vmaps'
               ...
               if(!has_permission('customers','','view') && !have_assigned_customers() && !has_permission('customers','','create')){
				...
				elseif($submenu->id == 'child-inventory')
				{
				   if(!has_permission('inventory_items','','view')  && !has_permission('inventory_items','','create'))
				   {
						continue;
			    ...
			    elseif($submenu->id == 'child-announcements')
				{ 
				   if(!has_permission('announcement_perm','','view'))
				   {
						continue;
