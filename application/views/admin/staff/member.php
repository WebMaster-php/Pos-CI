...
if($this->uri->segment(4))
{
	...
	$staff_permissions = json_decode(json_encode($staff_permissions), True);
}
?>
...
<div class="content">
   <div class="row">
      ...
      <?php } ?>
      <?php if(isset($member)){ 
         // echo "<pre>"; print_r($member->allow_ticket_filters); ?>
               ...
               <div class="tab-content">
                  <div role="tabpanel" class="tab-pane active" id="tab_staff_profile">
                     ...
                     <?php echo render_input('email','staff_add_edit_email',$value,'email',array('autocomplete'=>'off')); ?>
						...
						<div class="form-group">
						 ...
						 <select name="portfolio[]" id="portfolio" class="selectpicker" data-width="100%"  multiple  required>
								<?php if($portfolio)
								 {									
									foreach($allportfolio as $po){ ?>
											<option  value="<?php echo $po['name_id']; ?>"   name ="<?php echo $po['names']; ?>" 
								...
								<?php	}	
										...
										foreach($allportfolio as $po){ ?>
											<option  value="<?php echo $po['name_id']; ?>"  name ="<?php echo $po['names']; ?>"> <?php echo $po['names']; ?></option>
								<?php	} 
									} ?>	
						 </select>
						</div>
                     ...
                     <!-- ab -->
                     <?php
                     if($member->allow_ticket_filters == 1){?>
                     <h3 class="no-margin">
                     <?php echo _l('Settings'); ?>
                     </h3>
                     <p><?php echo _l('Defualt Ticket Display Tickets'); ?></p>
                     <?php $permis=unserialize($member->ticket_permissions); ?>
                     <div class="checkbox checkbox-primary">
                     ...
                     // echo "<pre>"; print_r($permis); exit('ijiji');  ?>
                     <div class="checkbox checkbox-primary">
                     <?php if(in_array($status['ticketstatusid'], $permis)){ ?>
                     <input type="checkbox" name="ticket_permissions[]" class="second" id="mass_delete" checked value="<?php echo $status['ticketstatusid']?>">
                     <?php 
                     }else{?>
                     <input type="checkbox" class="second" name="ticket_permissions[]" id="mass_delete" value="<?php echo $status['ticketstatusid']; ?>">
                     <?php }?>
                     ...
                     <div class="row">
                        <div class="col-md-12">
                           ...
                           <div class="checkbox checkbox-primary">
                              <input type="checkbox" 
                                       <?php if($member->tbl_staff_portfolio == '1'){ ?> checked <?php } ?> name="tbl_staff_portfolio" id="tbl_staff_portfolio" value="1">
                              <label for="tbl_staff_portfolio">Can Assign Portfolios</label>
                           </div>
                           <?php if(!isset($member)){ ?>
                     ...
                     <?php 
                        $hidden_items  = ' ' ;
                        if($member->hidden_items == 1){
                           $hidden_items = 'checked'; 
                        }
                     ?>

                     <!-- fake fields are a workaround for chrome autofill getting the wrong fields -->
                     <div class="checkbox checkbox-primary">
                              <input type="checkbox" name="hidden_items" id="hidden_items" value= "1" <?php echo $hidden_items ;?>>
                              <label for="hidden_items"><?php echo _l('hidden_items_view'); ?></label>
                     </div>
                     <div class="checkbox checkbox-primary">
                              <input type="checkbox" name="allow_ticket_filters" id="allow_ticket_filters" value= "1" <?php if($member->allow_ticket_filters == 1){ echo "checked";} ?>/>
                              <label for="allow_ticket_filters">Allow Ticket Filters</label>
                     </div>
                     <hr>
                     <h6><?php echo _l('lead_notify_heading'); ?></h6>
                           <div class="checkbox checkbox-primary">
                              <?php

                                 $notifyStatus = '';
                                 if(isset($member)) {
                                  if($member->email_notify_enable ==1){
                                   $notifyStatus = ' checked';
                                 }
                                 }
                                 ?>
                              <input type="checkbox" name="email_notify_enable" id="email_notify_enable" value = "1" <?php echo $notifyStatus; ?>>
                              <label for="email notification"><?php echo _l('lead_notify_label'); ?></label>
                           </div>
                     ...
                     <div class="form-group">
                        <?php
                        // $selected = $staff_inventory_notify[0]['source'];
                        // echo render_select('source',$sources,array('source_id','source_name'),'Source',$selected,array('required'=>'true')); 
                        ?>
                     </div>
                     <hr>
                     <div class="form-group">
                  ...
                  <div role="tabpanel" class="tab-pane" id="tab_staff_permissions">
                     ...
                     <div class="table-responsive">
						...
						<table id = "perm_table" class="table table-bordered roles no-margin" style = "display:none">
                           <thead>
                              <tr>
                              ...
                              <tr data-id="<?php echo $permission['permissionid']; ?>" data-name="<?php echo $permission['shortname']; ?>">
                                 ...
                                 <td class="text-center">
                                    <?php if($permission_condition['view'] == true){
                                       $statement = '';
                                       // if(has_permission($permission['shortname'],$member->staffid,'view_own')){
      ...
      <div class="btn-bottom-toolbar text-right btn-toolbar-container-out">
         <button type="submit" id = "saving_data" class="btn btn-info"><?php echo _l('submit'); ?></button>
      </div>
      <?php echo form_close(); ?>
      <?php if(isset($member)){ ?>
...
<script>
   ...
   $(function() {
	...
	var options = $('#portfolio > option:selected');
	if(options.length == 0)
	{
		$('table#perm_table').hide();
	}
	else{
		$('table#perm_table').show();
	}	
	...
	if(id){
		$.ajax({ 
					type : 'post',
					url : '<?php echo base_url();?>admin/portfolio/get_portfolio_permission',
					data: { value : a, id: id },
					success:function(dt){
							 $('table#perm_table').show();
							 $('tbody#check_pr').empty();
							 $("tbody#check_pr").html(dt);
						}
			 });	
		}
	else
