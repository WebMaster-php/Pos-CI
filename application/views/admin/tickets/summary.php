<div class="row">
 ...
 <div class="_filters _hidden_inputs hidden tickets_filters">
  ...
  echo form_hidden('ticket_follow');
  echo form_hidden('ticket_unassigned');
  if(is_admin()){
...
foreach($statuses as $status){
  $_where = '';
  if($where == ''){
    $_where = 'status='.$status['ticketstatusid'] .  ' AND tbltickets.portfolio_id = ' . $this->session->userdata('portfolio_id') ;
  } else{
    $_where = 'status='.$status['ticketstatusid'] . ' '. $where . ' AND tbltickets.portfolio_id = ' . $this->session->userdata('portfolio_id') ;
  }
  if(isset($project_id)){
    $_where = $_where . ' AND project_id='.$project_id . 'AND tbltickets.portfolio_id = ' . $this->session->userdata("portfolio_id");
  }
  ?>
