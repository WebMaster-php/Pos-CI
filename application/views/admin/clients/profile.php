            ...
            <table class="table dt-table">
               <thead>
                   <tr>
                     ...
                     <?php if(getAdmin() || has_permission('inventory','','delete') || has_permission('inventory','','edit')){ ?>
                     <th><?php echo _l('options'); ?></th>
                     <?php } ?>
                  </tr>
               </thead>
...
<script>
    ...
    function edit_inventory(e)
    {
        ...
        $.ajax({
            ...
            dataType:'json',
            success:function(dt){
                $("#inventory_id").val(dt[0].inventory_id);
                $("#account123").val(dt[0].account).change();
                //$("select[name='type_of_hardware']").val(dt[0].type_of_hardware).change();
                $("#serial_number").val(dt[0].serial_number);
                $("select[name='type_of_hardware']").val(dt[0].type_of_hardware).change();
                $("input[name='date_in']").val(dt[0].date_in);
                $("select[name='status']").val(dt[0].status).change();
                $("select[name='origin']").val(dt[0].origin).change();
                $("img.image").show();
                $("img.image").attr('src',dt[0].image);
                $("select[name='equipment_owner']").val(dt[0].equipment_owner).change();
                $("input[name='manufacturer']").val(dt[0].manufacturer);
                $("select[name='warranty_expiration_date']").val(dt[0].warranty_expiration_date).change();
                if(dt[0].warranty_expiration_date==10)
                {
                    $("#exp_no").val(dt[0].custome_nu);
                    $("#exp_type").val(dt[0].custome_type);
                    $("div#expiration_fields").show();
                }
                $("input[name='img_clone']").val(0);
                $("#description").val(dt[0].description);
                $("input[name='inventory_id']").val(dt[0].inventory_id);
                $("input[name='inventory_id']").val(dt[0].inventory_id);
                $("input[name='inventory_id']").val(dt[0].inventory_id);
            }
        });
