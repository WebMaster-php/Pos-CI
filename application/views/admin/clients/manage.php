                        ...
                        <a href="<?php echo admin_url('clients/client'); ?>" class="btn btn-info mright5 test pull-left display-block">
                            ...
                            <a href="<?php echo admin_url('clients/import'); ?>" class="btn btn-info pull-left display-block mright5 hidden-xs">
                                ...
                                <?php if(has_permission('customers','','view') || have_assigned_customers()) {
                                    $where_summary = ' ';
                                    $none = ''; 
									// $where_summary = 'AND portfolio_id = ' . $this->session->userdata('portfolio_id') ;
                                    // echo $where_summary; exit; 
                                    ...
                                    <div class="row mbot15">
                                        ...
                                        <div class="col-md-2 col-xs-6 border-right">
											<h3 class="bold">
                                                ...
                                                if($check)
                                                    { 
                                                        $none = 'cs.name = "none"';
                                                        $get_val = getCounterNone($none);

                                                        $clientExecptNone =total_rows('tblclients',($where_summary != '' ? substr($where_summary,5) : '')) - $get_val;
                                                        echo $clientExecptNone;
                                                    }
                                                   else{
                                                          echo total_rows('tblclients',$where_summary . ' portfolio_id = ' . $this->session->userdata('portfolio_id'));
                                                       }
                                                    ?>
                                            </h3>
                                        ...
                                        <div class="col-md-2 col-xs-6 border-right">
                                            <h3 class="bold">
                                                <?php 
                                                if($check){
                                                    $none = 'cs.name = "none" AND c.active = 1';
                                                    $get_val = getCounterNone($none );
                                                    echo total_rows('tblclients','active=1'.$where_summary )-$get_val; 
                                                }
                                                else{
                                                    echo total_rows('tblclients','active=1'.$where_summary . '  AND portfolio_id = ' . $this->session->userdata('portfolio_id'));
                                                }?>
                                        ...
                                        <div class="col-md-2 col-xs-6 border-right">
                                            <h3 class="bold">
                                                <?php 
                                                if($check){  
                                                    $none = 'cs.name = "none" AND c.active = 0';
                                                    $get_val = getCounterNone($none);
                                                    echo total_rows('tblclients','active=0'.$where_summary)-$get_val;
                                                }
                                                else{
                                                    echo total_rows('tblclients','active=0'.$where_summary  . '  AND portfolio_id = ' . $this->session->userdata('portfolio_id')); 
                                                }?>
                                            </h3>
                                        ...
                                        <div class="col-md-2 col-xs-6 border-right">
                                            <h3 class="bold"><?php 
                                                if($check){  
                                                    $none = 'cs.name = "none" AND c.active = 1';
                                                    $get_val_contact = getCounterNoneContacts($none);
                                                    echo total_rows('tblcontacts','active=1'.$where_summary)-$get_val_contact;
                                                    }
                                                else{
                                                    echo total_rows_con('tblcontacts','active=1'.$where_summary); 
                                                }?>
                                            </h3>
                                        ...
                                        <div class="col-md-2  col-xs-6 border-right">
                                            <h3 class="bold"><?php 
                                                if($check){  
                                                    $none = 'cs.name = "none" AND c.active = 0';
                                                    $get_val_contact = getCounterNoneContacts($none);
                                                    echo total_rows('tblcontacts','active=0'.$where_summary)-$get_val_contact;
                                                 }
                                                else{
                                                    echo total_rows_con('tblcontacts','active=0'.$where_summary);     
                                                }?>    
                                            </h3>
                                        ...
                                        <div class="col-md-2 col-xs-6">
                                            <h3 class="bold"><?php 
                                                if($check){  
                                                    $none = 'cs.name = "none" AND c.last_login LIKE "'.date('Y-m-d').'%"';
                                                    $get_val_contact = getCounterNoneContacts($none);
                                                    echo total_rows('tblcontacts','last_login LIKE "'.date('Y-m-d').'%"'.$where_summary) - $get_val_contact; 
                                                }
                                                else{
                                                    echo total_rows_con('tblcontacts','last_login LIKE "'.date('Y-m-d').'%"'.$where_summary);      
                                                }
                                                ?>
...
<script>
   ...
   //   $(document).load(function(){
           //alert('here');
           // var text_s =$('.dataTables_info').text(); 
           // alert(text_s);
           // console.log(text_s);         
        // });
