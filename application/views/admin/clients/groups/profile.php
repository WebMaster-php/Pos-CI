...
<?php 
// echo "<pre>"; print_r($client); exit('page'); ?>
   ...
   <div class="col-md-12">
      <ul class="nav nav-tabs profile-tabs row customer-profile-tabs" role="tablist">
        ...
        <!-- <li role="presentation">
         ...
         <li role="presentation">
            <a href="#cloud" aria-controls="cloud" role="tab" data-toggle="tab">
               <?php //echo  _l( ''); ?>Cloud
            </a>
         </li>
      ...
      <div class="tab-content">
        <div role="tabpanel" class="tab-pane<?php if(!$this->input->get('tab')){echo ' active';}; ?>" id="contact_info">
            <div class="row">
               ...
               <div class="col-md-6">
                    <!-- faisal code starts-->
                  <?php $portfolio_data_premission = get_portfolio_premission($this->session->userdata('staff_user_id')); if($portfolio_data_premission[0]->tbl_staff_portfolio == '1')
                  { ?>
                      <div class="form-group">
                        <label for="sel1">Select Portfolio </label>
                        <select class="form-control" id="portiflio_clients" name="portiflio_clients">
                        <?php foreach ($client_portfolio_data as $key => $portiflio_client) { ?>
                        <option <?php if($client->portfolio_id == $portiflio_client->name_id){ ?> selected <?php } ?> value="<?php echo $portiflio_client->name_id ?>"><?php echo $portiflio_client->names ?></option>
                        <?php } ?>
                        </select>
                      </div>
                  <?php } ?>
                 ...
                 <?php
                  // here
                $selected_serv = array();
               // echo "<pre>";  print_r($customer_services); exit; 
               if(isset($customer_services)){
                 foreach($customer_services as $service){
                    array_push($selected_serv,$service['serviceid']);
                 }
              }
              if(is_admin() ){
                echo render_select_with_input_services('services_in[]',$services,array('id','name'),'Versi Products',$selected_serv,'',array('multiple'=>true,'data-actions-box'=>true),array(''),'','',false);
                } 
                else {
                  echo render_select('services_in[]',$services,array('id','name'),'Versi Products',$selected_serv,array('multiple'=>true,'data-actions-box'=>true),array(),'','',false);
                }?> 
                <p id="company-error" class="text-danger" style= " display:none ">This field is required.</p>
      ...
      <div role="tabpanel" class="tab-pane<?php if($this->input->get('tab') && $this->input->get('tab') == 'contacts'){echo ' active';}; ?>" id="contacts">
         ...
         <!-- cloud -->
		  <div role="tabpanel" class="tab-pane" id="cloud">
            <h2 class="mtop5 mbot10"><b> VersiPOS </b></h2>
            <!-- <div class="col-xs-12 mobview">
            <div class="form-group">
            ...
            <table class="table ">
               ...
               <tbody>
                  <tr>
                     <td><input type="text" name="versiPOS_username" class = "form-control" value="<?php echo $client->versiPOS_username ?>"></td>
                      <td style="position: relative;">
                        <input type="password" name="versiPOS_password" id="versiPOS_password" class = "form-control" value="<?php echo $client->versiPOS_password ;?>">
                        
						  <i class="fa fa-eye eye_icon showval" onclick = 'pass( "versiPOS_password" )' ></i>
                      </td>

                     <td style="position: relative;">
                         <input type= "hidden" name = "versiPOS_merchant_id" id="versiPOS_merchant_id_hidden" value="<?php echo $client->versiPOS_merchant_id ?>"/>
						 <input type="password" name="versiPOS_merchant_id_not" id="versiPOS_merchant_id" class = "form-control" placeholder = " like 123"  value="<?php echo $client->versiPOS_merchant_id ?>"  onchange = "onchangeall('versiPOS_merchant_id', 'versiPOS_merchant_id_hidden')"/>
                        <i class="fa fa-eye eye_icon showval"  onclick = "showval('versiPOS_merchant_id', 'versiPOS_merchant_id_hidden','3' )"></i>
                     </td>
                     <td style="position: relative;">
						 <input type= "hidden" name = "versiPOS_store_id" id="versiPOS_store_id_hidden" value="<?php echo $client->versiPOS_store_id ?>">
                        <input type="password" name="versiPOS_store_id_not" id="versiPOS_store_id" class = "form-control" placeholder = " like 123" value="<?php echo $client->versiPOS_store_id ?>" onchange = "onchangeall('versiPOS_store_id', 'versiPOS_store_id_hidden')">
                        <i class="fa fa-eye eye_icon showval"  onclick = 'showval("versiPOS_store_id", "versiPOS_store_id_hidden", "2" )' ></i>
                      </td>
                  </tr>
               </tbody>
            </table>
            <h2 class="mtop5 mbot10"><b> VersiEats </b></h2>
            <table class="table">
               <thead>
                  <tr>
                     <td><b>Daily Report</b></td>
                     <td><b>Merchant IDs</b></td>
                  </tr>
               </thead>
               <tbody>
                  <tr>
                     <td><input type="text" name="versieats_daily_reports" class = "form-control" value="<?php echo $client->versieats_daily_reports ?>"></td>
                     <td><input type="text" name="versieats_merchant_id" class = "form-control" placeholder = " like 1,2,3" value="<?php echo $client->versieats_merchant_id ?>"></td>
                  </tr>
               </tbody>
            </table>
         </div>
            ...
            <table class="table dt-table">
               <thead>
                   <tr>
                     ...
                     <?php if(getAdmin() || has_permission('inventory','','delete') || has_permission('inventory_items','','edit')){ ?>
                     <th><?php echo _l('options'); ?></th>
                     <?php } ?>
                  </tr>
               </thead>
               <tbody>
                  ...
                  <tr>
                      ...
                      <td>
                          <?php 
                              if($c_admin['exp_date'] == 'No Warranty'){
                                 echo $c_admin['exp_date'];
                               }   
                                else{
                                  echo date('m/d/Y', strtotime($c_admin['exp_date']));
                                }
                            ?> 
                      </td>
                     <?php if(getAdmin() || has_permission('inventory','','edit')){ ?>
                     <td>
                        <?php if (getAdmin() || has_permission('inventory', '', 'edit')) { ?>
                        ...
                        <?php } ?>
                        <?php if (getAdmin() || has_permission('inventory', '', 'delete')) { ?>
...
window.onload = function() {
	changeval('versiPOS_merchant_id', 3); 
	changeval('versiPOS_store_id', 2); 
};
function changeval(e, ch){
	var passField = document.getElementById(e);
	passField.type="text";
