<div class="lead-wrapper" <?php if(isset($lead) && ($lead->junk == 1 || $lead->lost == 1)){ echo 'lead-is-junk-or-lost';} ?>>
   ...
   <?php } ?>
   <?php echo form_open((isset($lead) ? admin_url('clients/lead/'.$lead->id) : admin_url('clients/lead')),array('id'=>'cust_lead_form')); ?>
   <div class="row">
   ...
   <input type= "hidden"  name = "client_id" value="<?php echo $client_id;?>" />
      <div class="lead-view<?php if(!isset($lead)){echo ' hide';} ?>">
         ...
         <div class="col-md-4 col-xs-12 mtop15">
            ...
            <?php } ?>
             <!--Salik code start-->
            <?php  $assignedUsers=explode(',',$lead->assigned);
            $assigneNames=[];
            foreach ($assignedUsers as $key => $value) {
              $assigneNames[]=get_staff_full_name($value);
            }
            $nameofAssignedUsers=implode(',',$assigneNames);
            ?>
            <p class="text-muted lead-field-heading"><?php echo _l('lead_add_edit_assigned'); ?></p>
            <p class="bold font-medium-xs mbot15"><?php echo (isset($lead) && $lead->assigned != 0 ? $nameofAssignedUsers: '-') ?></p>

            <?php  $followedUsers=explode(',',$lead->followers);
            $followersNames=[];
            foreach ($followedUsers as $key => $value) {
              $followersNames[]=get_staff_full_name($value);
            }
            $nameofFollowedUsers=implode(',',$followersNames);
            ?>
            <p class="text-muted lead-field-heading"><?php echo _l('lead_add_edit_followers'); ?></p>
            <p class="bold font-medium-xs mbot15"><?php echo (isset($lead) && $lead->followers != 0 ? $nameofFollowedUsers: '-') ?></p>
      ...
      <div class="lead-edit<?php if(isset($lead)){echo ' hide';} ?>">
         ...
         <div class="col-md-3 mtop15">
            <?php
               ...
               $selected = (isset($lead) ? explode(',', $lead->assigned) : get_staff_user_id());
               ...
               foreach ($members as $key => $value) {
                $mul_select_data['id']=$value['staffid'];
                $mul_select_data['name']=$value['firstname'].' '.$value['lastname'];
                array_push($multiple_options, $mul_select_data);
              
               }
                echo render_select('assigned[]',$multiple_options,array('id','name'),'lead_add_edit_assigned',$selected,array('data-width'=>'105%','data-none-selected-text'=>'lead_add_edit_assigned','multiple'=>true,'data-actions-box'=>true),array(),'','',false); ?>
         </div>
         <div class="col-md-3 mtop15">
          
            <?php
               $followers_attrs = array();
               $followers_selected = (isset($lead) ? explode(',', $lead->followers) : get_staff_user_id());

               if(isset($lead)
                  && $lead->followers == get_staff_user_id()
                  && $lead->addedfrom != get_staff_user_id()
                  && !is_admin($lead->followers)
                  && !has_permission('leads','','view')
               ){
                 $followers_attrs['disabled'] = true;
               }
               
               $followers_multiple_options=[];
               $followers_data=$members;
               foreach ($followers_data as $key => $follower) {
                $mul_follow_select_data['id']=$follower['staffid'];
                $mul_follow_select_data['name']=$follower['firstname'].' '.$follower['lastname'];
                array_push($followers_multiple_options, $mul_follow_select_data);
              
               }
              
               echo render_select('followers[]',$followers_multiple_options,array('id','name'),'lead_add_edit_followers',$followers_selected,array('data-width'=>'105%','data-none-selected-text'=>'lead_add_edit_followers','multiple'=>true,'data-actions-box'=>true),array(),'','',false); ?>
         </div>
         <div class="clearfix"></div>
        ...
        <!-- <div class="row">
          ...
          <div class="col-md-6 mtop10">
            <div class="onoffswitch">
              <input type="checkbox" id="existing_customer" class="onoffswitch-checkbox" value="0" name="existing_customer">
              <label class="onoffswitch-label" for="existing_customer"></label>
            </div>
          </div>
        </div>
        </div>-->
        
        <div class="col-md-12" id="customers_records">
          <div class="form-group">
              <label for="" class="control-label">
              <small class="req text-danger">* </small>
              Select Existing Customer</label>
              
              <select class="selectpicker display-block"   data-live-search="true" data-width="100%" class="ajax-search" id="old_existing_customer" name="old_existing_customer" data-none-selected-text="<?php echo _l('Please Choose Customer'); ?>">
                
                <option ></option> <!--//-->
                <?php 
               // echo'<pre>';
              //print_r($followers);
                //$customersdata=$this->db->get('tblclients')->result();
                foreach($customersdata as $custdata){?>
                  <option value="<?php echo $custdata->userid; ?>" ><?php echo $custdata->company?></option>
                <?php } ?>
              </select>
