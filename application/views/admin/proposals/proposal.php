               ...
               <div class="panel-body">
                  <div class="row">
                     ...
                     <div class="col-md-6">
						...
						<label for="open_till" class="control-label"><small class="req text-danger">* </small> To</label>
							<select name="proposal_to" id="proposal_to" class="selectpicker" data-width="100%" data-none-selected-text="Nothing selected" tabindex="-98">
								<option value=""></option>
							</select>
						</div>
                        ...
                        <div class="row">
                           ...
                           <div class="col-md-6">
                              ...
                              <?php //echo render_input('email','proposal_email',$value); ?>
								...
								<div class="form-group" app-field-wrapper="open_till">
								<label for="open_till" class="control-label"><small class="req text-danger">* </small> Email</label>
									<select name="email" id="email" class="selectpicker" data-width="100%" data-none-selected-text="Nothing selected" tabindex="-98">
										<option value=""></option>
									</select>
								</div>
                           </div>
...
<script>
	$('body').on('change','#rel_id', function() {
		if($(this).val() != ''){
			var rel_id = $(this).val();
			rel_id = rel_id.split("_");
			console.log(rel_id[0]);
			$.get(admin_url + 'proposals/get_all_primary_contact/' + rel_id[0], function(result) {
				var proposal_to = '';
				var email = '';
				...
				if(result.length > 0){
					...
					for(var i=0; i<result.length; i++){
						proposal_to += '<option value="'+result[i].firstname+' '+result[i].lastname+'">'+result[i].firstname+' '+result[i].lastname+'</option>';
						email += '<option value="'+result[i].email+'">'+result[i].email+'</option>';
					}
					$('#proposal_to').append(proposal_to);
					$('#email').append(email);
		...
		if($('#rel_id').val() != ''){
			...
			$.get(admin_url + 'proposals/get_all_primary_contact/' + rel_id[0], function(result) {
				...
				if(result.length > 0){
					$('#proposal_to').empty();
					$('#email').empty();
					var selectedto =  '<?php if($proposal->proposal_to){echo $proposal->proposal_to;} ?>';
					var selectedemail =  '<?php if($proposal->email){echo $proposal->email;} ?>';
					var selectto = '';
					var selectemail = '';
					for(var i=0; i<result.length; i++){
						if(selectedto == result[i].firstname+' '+result[i].lastname){
							selectto = 'selected="selected"'
						}
						if(selectedemail == result[i].email){
							selectemail = 'selected="selected"'
						}
						proposal_to += '<option value="'+result[i].firstname+' '+result[i].lastname+'" '+selectto+'>'+result[i].firstname+' '+result[i].lastname+'</option>';
						email += '<option value="'+result[i].email+'" '+selectemail+'>'+result[i].email+'</option>';
					}
					$('#proposal_to').append(proposal_to);
					$('#email').append(email);
					$('.selectpicker').selectpicker('refresh');
   ...
   $(function(){
    ...
    $('body').on('change','#rel_id', function() {
     ...
     if($(this).val() != ''){
		var __rel_id = $(this).val();
			__rel_id = __rel_id.split('_');
			__rel_id = __rel_id[0];
      $.get(admin_url + 'proposals/get_relation_data_values/' + __rel_id + '/' + _rel_type.val(), function(response) {
		  //console.log(response);
		  
		  $('select[name="email"]').empty();
		  $("#proposal_to").empty();
		  
        //$('select[name="proposal_to"]').selectpicker('val',response.to);
        //$('#proposal_to').selectpicker('val',response.to);
		$("#proposal_to").append('<option value=""></option><option value="'+response.to+'" selected="selected">'+ response.to +' </option>');

		//$('#proposal_to').selectpicker('refresh');
        $('textarea[name="address"]').val(response.address);
        //$('select[name="email"]').selectpicker('val',response.email);
        
		$('select[name="email"]').append('<option value=""></option><option value="'+response.email+'"  selected="selected">'+ response.email +' </option>');
