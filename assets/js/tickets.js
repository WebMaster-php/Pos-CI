$(function() {
    ...
    $('.save_changes_settings_single_ticket').on('click', function(e) {
        ...
        var selectRequired = ['department', 'priority', 'contactid'];
    ...
    $('select[name="status_top"]').on('change', function() {
        var status = $(this).val();
		var customer_id = $("#client_customer_id").val();
		alert(customer_id);
        var ticketid = $('input[name="ticketid"]').val();
        requestGetJSON('tickets/change_status_ajax/' + ticketid + '/' + status + '/' + customer_id).done(function(response) {
