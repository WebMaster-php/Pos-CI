  ...
  $(function() {
	  ...
	  if ($('#project-files-upload-as').length > 0) {
          new Dropzone('#project-files-upload-as', $.extend({}, _dropzone_defaults(), {
              paramName: "file",
              uploadMultiple: true,
              parallelUploads: 20,
              maxFiles: 20,
              ...
              success: function(file, response) {
                  ...
                  if (this.getUploadingFiles().length === 0 && this.getQueuedFiles().length === 0) {
                      window.location.href = admin_url + 'sales/view/' + project_id + '?group=project_files';
                  }
              },
              sending: function(file, xhr, formData) {
                  formData.append("visible_to_customer", $('input[name="visible_to_customer"]').prop('checked'));
      ...
      init_rel_tasks_table_so(project_id, 'saleopp');
	  ...
	  initDataTable('.table-sale-discussions', admin_url + 'sales/discussions/' + project_id, [4], [4], 'undefined', [1, 'DESC']);
	...
	//bilal code start
    function milestones_switch_view_so() {
      $('#milestones-table').toggleClass('hide');
      $('.project-milestones-kanban').toggleClass('hide');
      if (!$.fn.DataTable.isDataTable('.table-milestones')) {
          initDataTable('.table-milestones', admin_url + 'sales/milestones/' + project_id, [2], [2]);
      }
	}
	//bilal code end
  ...
  function delete_project_discussion_so(id) {
      if (confirm_delete()) {
          requestGetJSON('sales/delete_discussion/' + id).done(function(response) {
              alert_float(response.alert_type, response.message);
              $('.table-sale-discussions').DataTable().ajax.reload(null, false);
          });
      }
  }
